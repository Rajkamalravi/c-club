    ...
    <div class="layout-wrapper chat-layout d-lg-flex bg-white <?=(!$eventtoken) ? 'non-event': '' ?>" data-bs-theme="light">
                ...
                <div class="flex-lg-column my-0 sidemenu-navigation">
                    <ul class="nav nav-pills side-menu-nav" role="tablist">
                        ...
                        <li class="nav-item d-lg-block channel-nav" role="channel">
                            <a class="nav-link" id="pills-channel-tab" data-bs-toggle="pill" href="#pills-channel" role="tab"
                             aria-selected="false" tabindex="-1"  data-toggle="tooltip" data-bs-toggle="tooltip" title="Channels">
        ...
        <div class="chat-leftsidebar height-unset">
            ...
            <div class="tab-content">
                ...
                <div class="tab-pane active show" id="pills-direct" role="tabpanel" aria-labelledby="pills-direct-tab">
                    <div class="d-flex align-items-center m-2">
                        <div class="d-flex align-items-center flex-grow-1">
                            ...
                            <div data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Refresh">
                                    <button type="button" class="btn btn-success btn-sm" onClick="taoh_load_network_entries();">
            ...
            <div class="chat-content d-lg-flex h-100">
                ...
                <div id="channel-chat-div" class="w-100 overflow-hidden position-relative bo-lf-rt h-100 " >
                    ...
                    <div id="zero-day-div" class="middle_block  position-relative d-none">
                        <!-- 0 day scenario will come here-->
                        <div class="d-flex flex-column align-items-center py-5" style="gap: 12px;">
                            <span class="fs-18 mb-5"></span><!-- You are the first on this networking.. Please wait for a while.... -->
                            <img class="no-network-place" src="<?php echo TAOH_CDN_MAIN_PREFIX; ?>/images/empty_network.png" width="300" alt="no-network">
                         </div>
                    </div>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#searchUser').on('keyup', function () {
                ...
                let visibleCount = 0;
                $('#dmChannelList li.channel-item').filter(function () {
                    let name = $(this).data('channel_name').toLowerCase();
        ...
        async function addParticipantToDmList(memberList) {
            ...
            if(memberList.length == 0) {
                $('#dmChannelList').html('<li class="text-center p-3">No users found</li>');
            ...
            for (const [ptoken, entry] of Object.entries(memberList)) {
                ...
                const userAvatarSrc = await buildAvatarImage(cell.avatar_image, fallbackSrc);
                const dmEntryHtml = `
                    <li class="channel-item" data-chatwith="${ptoken}" data-channel_name="${cell?.chat_name}">
                        <a class="d-flex align-items-center justify-content-between" href="javascript: void(0);">
                            <div class="channel_btn flex-grow-1 d-flex align-items-center overflow-hidden" title="${cell?.chat_name || 'User'}">
                                <div class="flex-shrink-0 me-2">
                ...
                if (liElem.find('.user-mood-status').length === 0) {
                    liElem.find('.channel_name').after(
...
<script>
            ...
            html += `<li id="channel-${channelId}" class="channel-item ${isActive ? 'active' : ''} ${(hasUpdates ==  1) ? 'have_updates' : ''} ${isTempChannel ? 'temp' : ''}" data-channel_passcode="" data-speed_networking="0" data-channel_ticket_slug="" data-name="channel" data-channel_id="${channelId}" data-channel_type="${channelType}" data-avatar_src="${userAvatarSrc}" data-channel_name="${channelName}" data-chatwith="${to_ptoken}">
                    <a class="d-flex align-items-center justify-content-between" href="javascript: void(0);">
                        <div class="channel_btn flex-grow-1 d-flex align-items-center overflow-hidden" title="${channelName}">
                            ...
                            <div class="flex-grow-1 overflow-hidden w-hide">
                                <span class="text-truncate mb-0 channel_name text-capitalize">${channelName} <i class="fas fa-info-circle text-secondary ml-2" data-chatwith="${to_ptoken}"></i></span>
                            </div>
                        </div>
                    </a>
                </li>`;
    ...
    async function send_message(channel_id, channel_type, message_text, is_background_send = 0) {

        message_text = convertMentionsToLinks(message_text);
        var taohAction = "taoh_ntw_send_message";
        const msg_timestamp = Date.now();

        let channelKey = `channel_${channel_id}`;
        let channelData = await IntaoDB.getItem(objStores.ntw_store.name, channelKey);
        let messages = channelData?.values?.messages || [];
        let lastMessageId = messages.length ? Number(messages[messages.length - 1].message_id) + 1 : 1;
    ...
    async function send_reply_message(channel_id, channel_type, message_text) {
        ...
        const parentMessageId = window.replyToMessageId;
        var replyCount = messages.filter(
            msg => msg.parent_message_id === parentMessageId && msg.deleted !== true
        ).length;
        replyCount += 1;
        ...
        saveLastSendMsgTimestamp(channel_id, msg_timestamp);
        render_messages(messageObj, "", 1, "reply");

        $(`.conversation-reply-count[data-id="${parentMessageId}"]`).removeClass('d-none').attr('data-count', replyCount).text(`${replyCount} ${replyCount > 1 ? "replies" : "reply"}`);
            ...
            if (Array.isArray(res.myaccept_ptoken) && res.myaccept_ptoken.length > 0) {
                for (const ptoken of res.myaccept_ptoken) {
                    ...
                    if (user) {
                        ...
                        <li class="channel-item" data-chatwith="${user.ptoken}" data-channel_name="${user.chat_name}">
                            <a class="d-flex align-items-center justify-content-between" href="javascript: void(0);">
            ...
            if (Array.isArray(res.request_ptoken) && res.request_ptoken.length > 0) {
                //if(speed_networking_last_timestamp != 0) {
                    ...
                    if (userInfo && !$('#connectModal').hasClass('show')) {
                        ...
                        $('#connectModal .accept_btn').attr('data-chatwith', myptoken).attr('data-chatfrom', req_ptoken).attr('data-action', 1).attr('data-chatfrom-name', pinned_by);
                        $('#connectModal .reject_btn').attr('data-chatwith', myptoken).attr('data-chatfrom', req_ptoken).attr('data-action', -1).attr('data-chatfrom-name', pinned_by);
    ...
    $(document).on('click', '.openchatacc', async function () {
        ...
        if (channelElem.length) {
            ...
            currentElem.html('<span class="spinner-border spinner-border-sm text-primary"></span>');
    ...
    $("#chat-send-btn").on("click", async function() {
        ...
        $("#chat_input").val("");
        await send_message(channelId, channelType, message);
    });

    $("#chat-reply-send-btn").on("click", async function() {
        ...
        await send_reply_message(channelId, channelType, text);
    });
    ...
    document.addEventListener('click', function (e) {
        ...
        if (link && link.parentElement.classList.contains('nav-item')) {
