...
if(empty($type)) {
    header('Location: ' . TAOH_SITE_URL_ROOT . '/directory/club');
}

if($type === 'flag') {
    $flag_slug = taoh_parse_url(3);

    $filter_tag = '';
    if (!empty($flag_slug) && ($type ?? '') === 'flag' && !empty($tag_category)) {
        foreach ($tag_category as $tags) {
            foreach ($tags as $item) {
                if (str_replace(' ', '-', strtolower($item)) === $flag_slug) {
                    $filter_tag = $item;
                    break 2;
                }
            }
    ...
    $page_heading = $filter_tag . ' Directory';

} elseif ($type === 'skill') {
    $skill_slug = taoh_parse_url(3);

    $page_heading = 'Skill Directory';
//    $page_heading = ucfirst($skill_slug) . ' Directory';
} elseif ($type === 'hobbies') {
    $hobby_slug = taoh_parse_url(3);

    $page_heading = 'Hobbies Directory';
} elseif ($type === 'company') {
    $company_slug = taoh_parse_url(3);

    $page_heading = 'Company Directory';
} elseif ($type === 'role') {
    $role_slug = taoh_parse_url(3);
    ...
    <div class="directory">
        ...
        <div class="col-md-11 m-auto">
            <div class="d-flex align-items-center dir-heading-wrapper justify-content-between">
                <h2 class="dir-heading text-left mt-3 mb-3"><i class="fa fa-folder mr-1"></i><?= $page_heading ?></h2>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            if (user_is_logged_in) {
                ...
                taoh_set_error_message('You need to be logged in to view the directory.', false, 'toast-middle-right', [
                    {
                        text: 'OK',
                        action: () => {
                            window.location.href = _taoh_site_url_root + '/login';
                        },
        ...
        function handleDirectoryErrors(errorCode, selectedFlag = '') {
            let title = 'Warning';
            let content = 'Something went wrong. Please try again later.';
            let buttons = {};

            switch (errorCode) {
                case 'not_exist_in_my_profile_flags':
                    content = `To view this directory, you need to add the ${selectedFlag} related flag to your profile. Please update your profile flags in settings.`;
                    buttons = {
                        goto_setting: {
                            text: 'Go to Settings',
                            action: () => window.location.href = `${_taoh_site_url_root}/settings/3`
                        },
                        confirm: {
                            text: 'OK',
                            action: () => window.location.href = `${_taoh_site_url_root}/directory/club`
                        }
                    };
                    break;
                case 'no_flags_in_my_profile':
                    content = 'No flags are set in your profile. Please update your profile flags in settings.';
                    buttons = {
                        goto_setting: {
                            text: 'Go to Settings',
                            action: () => window.location.href = `${_taoh_site_url_root}/settings/3`
                    ...
                    break;
                case 'no_flag_selected':
                    content = 'No flag selected. Please select a valid flag to view the directory.';
                    buttons = {
                        confirm: {
                            text: 'OK',
                            action: () => window.location.href = `${_taoh_site_url_root}/directory/club`
                    ...
                    break;
                case 'no_flags_in_directory':
                    content = 'No flags available in the directory. Please try again later.';
                    buttons = {
                        confirm: {
                            text: 'OK',
                            action: () => window.location.href = `${_taoh_site_url_root}/directory/club`
                    ...
                    break;
                default:
                    content = selectedFlag
                        ? `${selectedFlag} directory is not available.`
                        : 'Something went wrong while fetching profiles. Please try again later.';
                    buttons = {
                        confirm: {
                            text: 'OK',
                            action: () => window.location.href = `${_taoh_site_url_root}/directory/club`
            ...
            let buttonArray = Object.values(buttons).reverse().map((button, index) => ({
                text: button.text,
                action: button.action,
                class: `dojo-v1-btn float-right mt-3 mb-3 ${index > 0 ? 'mr-2' : ''}`
            }));

            taoh_set_warning_message(content, false, 'toast-middle', buttonArray);

            // jq_confirm_alert(title, content, 'orange', buttons);
        }
        ...
        function loadProfiles(callFromEvent = 'init') {
            ...
            let data = {
                'taoh_action': 'taoh_dir_users_list',
                'search': q,
                'view': 'profile',
                'type': type ?? '',
            };
            ...
            if(data.type === 'flag') {
                let selectedFlag = '<?= $filter_tag ?? ''; ?>';
                if(selectedFlag) {
                    data.flag = selectedFlag;
                }
            } else if(data.type === 'skill') {
                data.skill = '<?= $skill_slug ?? ''; ?>';
            } else if(data.type === 'hobbies') {
                data.hobby = '<?= $hobby_slug ?? ''; ?>';
            ...
            $.ajax({
                ...
                success: function (response, textStatus, jqXHR) {
                    if (response.success) {
                        ...
                        handleDirectoryErrors(response.error, selectedFlag);
                    }
                },
        ...
        async function appendProfiles(response, callFromEvent = 'init') {
            ...
            if (response.success && response.output?.length > 0) {
                ...
                for (const userInfo of response.output) {
                    ...
                    // const siteInfo = userInfo.site || {};
                    const siteSource = _taoh_site_url_root; // siteInfo.source || _taoh_site_url_root;
