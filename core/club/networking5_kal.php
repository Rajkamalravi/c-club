                        ...
                        <div class="col-lg-12">
                            <div class="row align-items-center"> 
                                ...
                                <div class="col-md-10 d-flex breadcrumb-list-flow" style="padding-right: 190px;">                          
                                    ...
                                    <a style="float:right;" target="_blank" href="<?php echo TAOH_MASTER_NETWORKING_URL; ?>" 
                                    class="btn btn-primary btn-sm mt-2 w-10" id="go_to_masternetworking_room">Go to Master Networking Room</a>
                                    <?php } ?>
                                </div>       
                            </div>
                        </div>
    ...
    <div class="layout-wrapper chat-layout d-lg-flex bg-white <?=(!$eventtoken) ? 'non-event': '' ?>" data-bs-theme="light">
                ...
                <div class="flex-lg-column my-0 sidemenu-navigation">
                    <ul class="nav nav-pills side-menu-nav" role="tablist">
                        ...
                        <li class="nav-item d-lg-block direct-nav" role="direct">
                            <a class="nav-link active" id="pills-direct-tab" data-bs-toggle="pill" href="#pills-direct" role="tab" aria-selected="false"
                        ...
                        <li class="nav-item d-lg-block channel-nav" role="channel">
                            <a class="nav-link" id="pills-channel-tab" data-bs-toggle="pill" href="#pills-channel" role="tab" aria-selected="false" 
        ...
        <div class="chat-leftsidebar height-unset">
            ...
            <div class="tab-content">
                ...
                <div class="tab-pane active show net-chat-search" id="pills-direct" role="tabpanel" aria-labelledby="pills-direct-tab">
                    <div class="d-flex align-items-center m-2">
                        <div class="d-flex align-items-center flex-grow-1">
                            ...
                            <div data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Refresh" class="refresh-icon-net">
                                    <button type="button" class="btn btn-success btn-sm p-2" onClick="taoh_load_network_entries();">
            ...
            <div class="chat-content d-lg-flex h-100">
                ...
                <div id="start-screen" class="w-100 overflow-hidden position-relative bo-lf-rt h-100 start-screen" >
                    <div class="d-flex flex-column align-items-center py-5" style="gap: 12px;">
                        <span class="fs-18 mb-5 text-center zero_day_label d-none">Click any participant name on the left to start the converations</span>
                ...
                <div class="networking-sidebar participants-sidebar ovrflw-y" id="participants-sidebar" style="height: 100%; display: none;">
                    ...
                    <div class="border-con p-3 mb-3 d-none d-xl-block">
                        ...
                        <div class="d-flex align-items-center flex-wrap" style="gap: 12px;">
                            <?php if($keyword != "dm") { ?>
                            <button id="tourbutton" type="button" class="btn bor-btn d-xl-block">Start Application Tour</button>
                            <?php } ?>
    ...
    <script type="text/javascript">
        ...
        async function addParticipantToDmList(memberList) {
            ...
            for (const [ptoken, entry] of Object.entries(memberList)) {
                ...
                console.log("ticket_details ticket_details", ticket_details);
                ...
                const userAvatarSrc = await buildAvatarImage(cell.avatar_image, fallbackSrc);
                const dmEntryHtml = `
                    <li class="openProfileSideBar comm-participants" data-chatwith="${ptoken}" data-all-data="${cell?.chat_name+"~"+cell?.type+"~"+cell?.full_location+"~"+companies+"~"+roles+"~"+skillContent+"~"+ticket_type_name}" data-avatar_src="${userAvatarSrc}" data-channel_type="${taohChannelDm}" data-channel_name="${cell?.chat_name}" data-ticket_type_name="${ticket_type_name}">
                        <a class="d-flex align-items-center justify-content-between" href="javascript: void(0);">
                            <div class="channel_btn flex-grow-1 d-flex align-items-center overflow-hidden" >
                                ...
                                <div class="flex-grow-1 overflow-hidden w-hide dm-user-info-icon" data-chatwith="${ptoken}">
                                    <span class="text-truncate mb-0 chat_name text-capitalize mr-2" >${cell?.chat_name || 'User'} </span>  <i class="fa-solid fa-circle-user text-capitalize mt-1 mr-1" data-toggle="tooltip" data-bs-toggle="tooltip" title="${capitalize(profileTitle)}" style="color: ${getProfileColor(profileTitle)}; float: right;"></i> ${ticket_type_name != "" ? `<i class="fa-solid fa-ticket mr-2 mt-1" data-toggle="tooltip" data-bs-toggle="tooltip" title="${capitalize(ticket_type_name)}" style="color: ${getTicketColor(ticket_type_name)}; float: right;"></i>` : ''}
                                </div>                    
                            </div>
                        </a>
                    </li>
                ...
                let userIcon = `<i class="fa-solid fa-circle-user mt-1 mr-1"
                data-bs-toggle="tooltip"
                title="${capitalize(profileTitle)}"
                style="color:${getProfileColor(profileTitle)}; float: right;"></i>`;

                let ticketIcon = ticket_type_name
                ? `<i class="fa-solid fa-ticket mr-2 mt-1"
                        data-bs-toggle="tooltip"
                        title="${capitalize(ticket_type_name)}"
                        style="color:${getTicketColor(ticket_type_name)}; float: right;"></i>` : '';

                let chatNameHtml = `
                <span class="text-truncate mb-0 chat_name text-capitalize mr-2">
                ${cell?.chat_name || 'User'}
                </span>
                ${userIcon}
                ${ticketIcon}
                ...
                const liElem = $(`li[data-chatwith="${ptoken}"]`);
                if ($(`#dmChannelList li[data-chatwith="${ptoken}"]`).length == 0) {
                    $('#dmChannelList').append(dmEntryHtml);
                } else {
                    const newChatName = cell?.chat_name || 'User';
                    liElem.find('.chat_name').text(newChatName);
                    liElem.attr('data-channel_name', newChatName);
                }
                if (liElem) {
                    liElem.find('.chat_name_div').html(chatNameHtml);
                    liElem.attr('data-all-data', `${cell?.chat_name+"~"+cell?.type+"~"+cell?.full_location+"~"+companies+"~"+roles+"~"+skillContent+"~"+ticket_type_name}`);
                    liElem.attr('data-ticket_type_name', ticket_type_name);
                }

                if(liElem.find('.ticket-type-text').length === 0 && ticket_type_name !='' && ticket_type_name != null && ticket_type_name != undefined) {
                    liElem.find('.chat_name').after(
                        `<br><span class="mb-0 fs-12 text-truncate ticket-type-text" style=" border-radius: 5px; color: #877d7d;">
                        <i class="fa-solid fa-briefcase"></i> ${roles}                        
                        <i class="fa-solid fa-building ml-1"></i> ${companies} </span>`
                    );
                }

                if (liElem.find('.user-mood-status').length === 0 && userMoodStatus && userMoodStatus.trim() !== '') {
                    liElem.find('.chat_name').after(
                        `<span title="${stripHtml(userMoodStatus)}" class="mb-0 fs-12 text-truncate user-mood-status" style=" border-radius: 5px;display: block; font-style: italic;">${userMoodStatus}</span>`
                    );
                }

                // $(`li[data-chatwith="${ptoken}"] .channel_name`).after(
...
<script>
    ...
    async function renderDMChannelList(channelArray = []) {
        ...
        console.log("DM updateUnreadCount Part");        
    ...
    function updateUnreadCount(type) {
        ...
        if(type == 'channel') {
            var count = $('#myChannelList .have_updates').length + $('#channelList .have_updates').length + $('#exhibitorChannelList .have_updates').length + $('#sessionChannelList .have_updates').length;
        } else if(type == 'direct') {
            var count = $('#dmChannelList .have_updates').length;
        }
    ...
    async function updateProfileInfo(userInfo, userLiveStatus, fromParticipants = 0) {
        ...
        //} else {
                ...
                <div class="">
                    <div class="user-profile-img">
                        <div class="p-v1-header">
                            ...
                            ${ticketTypeByPtoken[userInfo.ptoken] ? `<p class="fs-14 text-truncate user-profile-status mt-1 mb-0"><i class="fa-solid fa-ticket"></i> ${ticketTypeByPtoken[userInfo.ptoken]}</p>` : ''}
            ...
            userProfileData += `${(titleValue != "" && companyValue != "") ? `<div class="p-v1-card">
                    <h5 class="p-v1-title">
                        <i class="fa-solid fa-briefcase"></i> 
                        <span>Role, Company</span>
                    </h5>
                    <p class="p-v1-content">${titleValue}, ${companyValue}</p>
                </div>` : ''}`;
