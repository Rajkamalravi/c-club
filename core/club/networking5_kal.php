...
$home_link = $room_data['home_link'] ?? TAOH_SITE_URL_ROOT . '/club';
...
?>
    ...
    <style>
        ...
        .chat-layout .channel-session:last-child {
        ...
        .net-left-fixed .side-menu .side-menu-nav {
            padding: 7px 0;
        }
        #chat-reply-conversation-list li a {
            color: #000;
        }
        .chat-layout .chat-reply-sidebar, .chat-layout .channelData-sidebar {
            height: calc(100vh - 71px)
        }    
        #browse_allchannels_channels .channel-item.px-lg-4 {
        ...
        .chat-leftsidebar.watchPartyEnabled #pills-channel .channel_btn > .flex-shrink-0:nth-child(1) {
            margin-right: 0 !important;
        }
        .chat-leftsidebar.watchPartyEnabled #pills-channel .channel_btn {
            justify-content: center;
        }
        .chat-leftsidebar.watchPartyEnabled #pills-channel .channel_btn > .w-hide, 
        .chat-leftsidebar.watchPartyEnabled #pills-channel .channel_btn div:nth-child(2) {
            display: none !important;
        }
        .chat-leftsidebar.watchPartyEnabled #pills-channel div:nth-child(2) > .w-hide {
            display: none !important;
        }
        .chat-leftsidebar.watchPartyEnabled #pills-channel .watch-enable-no, 
        .chat-leftsidebar.watchPartyEnabled #pills-channel .channel_btn + .dropdown {
            display: none;
        }
        .chat-leftsidebar.watchPartyEnabled #pills-channel .chat-list li > a {
            padding: 10px 15px;
        }
        .chat-leftsidebar.speed {
        ...
        .collapsible.open {
            max-height: 525px; /* adjust based on expected content size */
        }
    ...
    .layout-wrapper.watchPartyEnabled .net-left-fixed {
        width: 160px;
    }
    .layout-wrapper.watchPartyEnabled .user-chat#user-chat {
        left: 160px !important;
        width: calc(100% - 160px) !important;
    }
    .networking-mobile-sticky.sticky-top {
        position: fixed;
        width: 100%;
    }
    .channelList.list-unstyled.chat-list.chat-user-list {
       /*  max-height: 300px; */
        overflow: auto;
    }
    #pills-speed-tab.active .pills-gray-icon, #pills-speed-tab .pills-white-icon {
        display: none;
    }
    #pills-speed-tab.active .pills-white-icon {
        display: inline-block;
    }
    .chat-input-section {
        background-color: rgba(255, 255, 255, 1);
    }   
    .chat-layout .list-scroll-onmobile {
        max-height: calc(100vh - 300px) !important;
        overflow-y: auto;
        scrollbar-width: thin;
    }
    #go_to_masternetworking_room {
        float: right;
        position: fixed;
        right: 20px;
        top: 12px;
    }
    #broadcast_message_btn {
        float: right;
        position: fixed;
        right: 260px;
        top: 12px;
    ...
    #pills-channel .left-section 
    .chat-conversation .right .conversation-list .conversation-name {
        align-items: baseline;
    }
    .openchatacc.have_updates .text-truncate {
        display: inline-block;
    }
    ...
    .profile-tooltip + .tooltip .tooltip-inner {
        text-align: left;
        max-width: 350px;
        white-space: normal;
    }
    ...
    @media (min-width: 1199.98px) {
        ...
        .chat-layout .chat-reply-conversation, .chat-layout .chat-like-conversation {
            height: calc(100vh - 215px);
        }
        .non-event.chat-layout .chat-leftsidebar {
            height: calc(100vh - 71px);
        }
        .chat-layout .user-profile-sidebar, .chat-layout .channelData-sidebar {
            height: calc(100vh - 71px);
        } 
        .chat-layout .user-profile-desc {
        ...
        .chat-layout .chat-leftsidebar.height-unset {
    ...
    @media (min-width: 992px) {
        ...
        .side-menu-nav .nav-item.mobile-nav-active {
            display: none;
        }
        .chat-conversation#chat-conversation-channel .chat-conversation-list {
            margin-bottom: 60px;
        }
        .mobile-flex div {
            display: none
    ...
    @media (max-width: 991.98px) {
        ...
        .user-chat.user-chat-show#user-chat {
            visibility: visible;
        ...
        .layout-wrapper.hideSideBar .user-chat#user-chat {
            width: 100% !important;
        }
        #speed_networking .user-chat-topbar .d-xl-none a.user-chat-remove {
            display: none;
        }
        .chat-conversation {
        ...
        #channel-chat.watchPartyEnabled .chat-conversation {
            height: calc(100vh - 225px);
            margin-bottom: 0;
        }
        #channel-chat.hideSideBar .chat-conversation {
            height: calc(100vh - 225px);
            margin-bottom: 0;
        }
        #channelData-sidebar.networking-sidebar {
            position: fixed;
        }
        #start-screen {
            border: 0 !important;
        }
        #channel-chat-div #browse_channels_wrapper > div > div > div {
            vertical-align: middle;
            margin-top: 10px;
        }
        .video-section-open-block {
            height: calc(100vh - 157px);
            overflow-x: hidden;
            scrollbar-width: thin;
            display: block !important;
        }
        .video-section-open-block #channel-chat #chat-conversation-channel {
            height: auto;
        ...
        .sn-moreinfo-sidebar.participants-sidebar {
            position: fixed;
            max-width: none !important;
            padding-bottom: 30px;
        }
        ...
        .user-chat-nav .nav-btn.user-profile-show {
            line-height: 33px;
        }
        #chat-reply-sidebar #chat-reply-input-container {
            bottom: 66px;
        }
        #chat-reply-sidebar.chat-reply-sidebar {
           /* min-width: 400px;
            max-width: 400px; */ 
        }
        ...
        #speed_networking .zeroday-speed {
            height: calc(100vh - 140px);
            overflow: auto;
            scrollbar-width: thin;
        }
        #channel-chat-div.channel-chat-div-wp {
            overflow-y: auto !important;
            scrollbar-width: thin;
            height: calc(100vh - 70px) !important;
        }
        #channel-chat-div.channel-chat-div-wp.channel-chat-div-reply {
            height: 100% !important;
            overflow: hidden !important;
        }
        .channel-chat-div-wp .chat-input-bottom, .channel-chat-div-wp .chat-input-section {
            position: static;
        }
        .channel-chat-div-wp .chat-input-bottom {
            padding-bottom: 50px;
        }
        .channel-chat-div-wp .video-section-open-block {
            height: auto;
        }
        .channel-chat-div-wp .replyCard {
            position: static;
            display: none;
        }
        .channel-chat-div-wp.channel-chat-div-wp-reply .replyCard {
            position: absolute;
            display: block;
        }
        .chat-layout .list-scroll-onmobile {
        ...
        .speed_networking_div #contentCarousel {
            margin-bottom: 80px;
        }
        .zeroday-speed .card {
            margin: initial;
        }
        .all-channels .channel-lists .ch-list:last-child, .all-channels .channel-lists .exh-list:last-child {
            margin-bottom: 50px;
        }
        .user-profile-desc .simplebar-content {
            margin-bottom: 30px;
        }
        #browseMoreChannels {
            margin-bottom: 50px;
    ...
    <div class="bg-white" >
        <header class="networking-mobile-sticky sticky-top bg-white border-bottom border-bottom-gray">
            <section class="hero-area bg-white shadow-sm overflow-hidden">
                ...
                <div class="container" style="margin:10px 0;padding-left:20px;height:50px">
                    <?php
                    ...
                    <div class="hero-content align-items-center justify-content-between">
                        <div class="col-lg-12">
                            <div class="row align-items-center"> 
                                ...
                                <div class="col-md-10 d-flex breadcrumb-list-flow" style="padding-right: 190px;">                          
                                    <ul class="nav nav-tabs justify-content-left border-0 <?=($room_app == "live" ? 'd-none' : '')?>" role="tablist">
                                        <?php
                                        ...
                                        foreach ($breadcrumbs as $value) {
                                            ...
                                            if($link) {
                                                $link = TAOH_SITE_URL_ROOT.$link;
                                            } else {
                                                $link = $link;
                                            }

                                            echo '<li class="test nav-item"  ' . $count . '>';
                                        ...
                                        if ($isEventNetworking && !empty($msg_from_owner)) {
                                            ...
                                            // 1️⃣ URL decode (this is missing in your code)
                                            $msg_from_owner = urldecode($msg_from_owner);

                                            // 2️⃣ Decode HTML entities (do it twice because it's double-encoded)
                                            $msg_from_owner = html_entity_decode($msg_from_owner, ENT_QUOTES | ENT_HTML5, 'UTF-8');
                                            $msg_from_owner = html_entity_decode($msg_from_owner, ENT_QUOTES | ENT_HTML5, 'UTF-8');

                                            // 3️⃣ Remove all HTML tags
                                            $msg_from_owner = strip_tags($msg_from_owner);

                                            // 4️⃣ Remove non-breaking spaces & zero-width chars
                                            $msg_from_owner = preg_replace('/[\x{00A0}\x{200B}-\x{200D}\x{FEFF}]/u', ' ', $msg_from_owner);

                                            // 5️⃣ Normalize whitespace
                                            $msg_from_owner = trim(preg_replace('/\s+/', ' ', $msg_from_owner));

                                            // 6️⃣ Remove duplicate sentences
                                            $sentences = array_unique(array_filter(array_map(
                                                'trim',
                                                preg_split('/(?<=\.)\s+/', $msg_from_owner)
                                            )));
                                            $msg_from_owner = implode(' ', $sentences);

                                            // 7️⃣ Escape ONCE for tooltip
                                            $msg_from_owner = htmlspecialchars($msg_from_owner, ENT_QUOTES, 'UTF-8');
                                        
                                            echo '<li class="nav-item ml-2"><span>| Note from organizer</span>';
                                            echo '<i class="fa fa-info-circle ml-2 cursor-pointer note-tooltip" style="color: #15a4f7; font-size: 20px;" data-bs-toggle="tooltip" title="'.(($msg_from_owner)).'"></i>';
                                            echo '</li>';
                                        }
                                    ...
                                    <?php 
                                    if(ORGANIZER_CHANNEL_ENABLE && ($event_owner_ptoken == $my_ptoken || in_array($my_ptoken,$owner_org_ptoken))) { ?>
                                        <a style="float:right;" target="_blank" class="btn btn-primary btn-sm mt-2 w-10" id="broadcast_message_btn">Broadcast Message</a>
                                        <a style="float:right;" target="_blank" href="<?php echo TAOH_MASTER_NETWORKING_URL; ?>" class="btn btn-primary btn-sm mt-2 w-10" id="go_to_masternetworking_room">Go to Master Networking Room</a>
                                    <?php } ?>
                                </div>
                            </div>
                        </div>
    ...
    <div class="layout-wrapper chat-layout d-lg-flex bg-white <?=(!$eventtoken) ? 'non-event': '' ?>" data-bs-theme="light">
        ...
        <!-- start leftsidebar-menu -->
            <div class="side-menu flex-lg-column">
                ...
                <div class="flex-lg-column my-0 sidemenu-navigation">
                    <ul class="nav nav-pills side-menu-nav" role="tablist">
                        <li class="nav-item mobile-nav-active">
                            <a class="nav-link mb-1" id="back-to-home-tab" data-bs-toggle="home" href="<?=$home_link?>" role="tab">
                                <i class="fa-solid fa-arrow-left"></i>
                            </a>
                        </li>
                        
                        <!-- <li class="nav-item d-lg-block" role="participant">
                            <a class="nav-link active" id="pills-participant-tab" data-bs-toggle="pill" href="#pills-participant" role="tab" aria-selected="false" tabindex="-1">
                                <i class="fa align-middle fa-group"></i>
                            </a>
                        </li> -->
                        <?php if($room_app != "dm") { ?>
                        <li class="nav-item d-lg-block channel-nav" role="channel">
                            <a class="nav-link active" id="pills-channel-tab" data-bs-toggle="pill" href="#pills-channel" role="tab" aria-selected="false" 
                           tabindex="-1"  data-toggle="tooltip" data-bs-toggle="tooltip" title="Channels">
                        ...
                        <li class="nav-item d-lg-block speed-nav" role="speed">
                            ...
                            tabindex="-1"  data-toggle="tooltip" data-bs-toggle="tooltip" title="Speed Networking" >
                               <!-- <span class="avatar-title bg-white rounded-circle border text-dark">
                                ...
                                <svg width="35" height="35" viewBox="0 0 205 220" class="pills-gray-icon" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <svg width="35" height="35" viewBox="0 0 205 220" class="pills-white-icon" xmlns="http://www.w3.org/2000/svg">
                                
                                <!-- <circle cx="100" cy="100" r="90" fill="#f8f9fa"  stroke="#878a92" stroke-width="12"/> -->
                                <path d="M110 30 L110 95 L150 95 L90 170 L90 105 L50 105 Z"
                                        fill="#ffffff" stroke="#ffffff" stroke-width="12" stroke-linejoin="round" stroke-linecap="round"/>
                                </svg>

                            </a>
                        </li>                        
                        <?php } ?>
                        <li class="nav-item d-lg-block direct-nav" role="direct">
                            <a class="nav-link" id="pills-direct-tab" data-bs-toggle="pill" href="#pills-direct" role="tab" aria-selected="false"
                             tabindex="-1" data-toggle="tooltip" data-bs-toggle="tooltip" title="Chats" >
                                <i class="fa-solid fa-message"></i>
                            </a>
                        </li>
                        <?php 
                        if($event_owner_ptoken != $my_ptoken && !in_array($my_ptoken,$owner_org_ptoken)) { 
                            $chat_with_org = true; ?>
                        <li class="nav-item  chat_with_organizer_div d-none" role="org">
                            <a class="nav-link chat_with_organizer bg-warning" data-toggle="tooltip" data-bs-toggle="tooltip" title="Chat with Organizer" data-channel_name="Organizer" 
                            href="#pills-org" role="tab1" aria-selected="false" tabindex="-1" style="color: #fff;">
                                <i class="fa align-middle fa-user"></i>
                            </a>
                        </li> 
                        <?php } else { $chat_with_org = false; } ?>
                        <li class="nav-item status_modal_btn <?= !$chat_with_org ? 'mt-lg-auto' : '' ?>" role="presentation">
                           ...
                           tabindex="-1"  data-toggle="tooltip" data-bs-toggle="tooltip" title="Status">
                                <svg width="35" height="35" viewBox="0 0 72 80" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="chat-leftsidebar height-unset">
            ...
            <div class="tab-content">
                ...
                <!-- Start participant tab-pane -->
                <div class="tab-pane d-none" id="pills-participant" role="tabpanel" aria-labelledby="pills-participant-tab">
                        <!-- Start Participant -->
                ...
                <!-- End participant tab-pane -->

                <!-- Start speed tab-pane -->
                <div class="tab-pane" id="pills-speed" role="tabpanel" aria-labelledby="pills-speed-tab">
                ...
                <!-- Start channel tab-pane -->
                <div class="tab-pane <?php if($room_app != "dm") { echo "active show"; } ?> " id="pills-channel" role="tabpanel" aria-labelledby="pills-channel-tab">
                                        ...
                                        <div class="flex-grow-1">
                                            <h4 class="mb-0 fs-11 text-muted text-uppercase">
                                                ...
                                                <button class="btn shadow-none border-0 py-0" type="button" data-bs-toggle="collapse" data-bs-target="#myChannelCollapse" aria-expanded="false" aria-controls="myChannelCollapse">
                                                    <i id="myChannelCaret" class="fa-solid fa-caret-up"></i>
                                                </button> 
                                            </h4>
                                        </div>                                        
                                    </div>
                                        ...
                                        <div class="flex-grow-1">
                                            <h4 class="mb-0 fs-11 text-muted text-uppercase">
                                                ...
                                                <button class="btn channel-collapse-icon shadow-none border-0 py-0" type="button" data-bs-toggle="collapse" data-bs-target="#channelCollapse" aria-expanded="false" aria-controls="channelCollapse">
                                                    <i id="ChannelCaret" class="fa-solid fa-caret-up"></i>
                                                </button>
                                            </h4>
                                        </div>
                                        <div class="flex-shrink-0">
                                            <div>
                                                ...
                                                <!-- Button trigger modal -->
                                                <button data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Refresh" type="button" class="btn btn-success btn-sm p-2 mr-1" id="channel_refresh">
                                                    <i class="fa fa-refresh align-middle"></i>
                                                </button>
                                                ...
                                                <?php if($create_channel_flag == 1){ ?>
                                                    ...
                                                    class="create_channel btn btn-success btn-sm p-2">
                ...
                <div class="tab-pane <?php if($room_app == "dm") { echo "active show"; } ?>  net-chat-search" id="pills-direct" role="tabpanel" aria-labelledby="pills-direct-tab">
                    <div class="d-flex align-items-center m-2">
                        <div class="d-flex align-items-center flex-grow-1">
                            <div class="form-group me-2 flex-grow-1 mb-0">
                                <input name="query" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Search by employer, professional or service provider" class="form-control form--control ps-3 alphanumericInput" 
                                    type="text" id="searchUser" placeholder="Professional or Employer" autocomplete="off" />
                                <!-- <span class="la la-search input-icon"></span> -->
                                
                             <div class="search-btn-box mb-2">
                                 <button style="padding:6px 10px" class="btn search theme-btn" id="search" type="submit">
                                    <i class="la la-search"></i>
                                </button>
                            </div>
                            </div>
                            
                            <div data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Refresh" class="refresh-icon-net">
                                    <button type="button" class="btn btn-success btn-sm p-2" id="dm_refresh" >
                    ...
                    <div class="chat-message-list">
                        <div class="collapse show" id="dmCollapse">
                            <div class="extra-view-scroll" style="min-height: unset;max-height: unset;">
                            <ul class="dmChannelList list-unstyled chat-list chat-user-list" id="dmChannelList">

                            </ul>
                            <div id="noResultMsg" style="display:none; padding:10px; text-align:center; color:#888;">
                                No users found
                            </div>
                            </div>
                        </div>
                ...
                <!-- Start org tab-pane -->
                <div class="tab-pane" id="pills-org1" role="tabpanel" aria-labelledby="pills-org-tab1">
        ...
        <div class="user-chat w-100 overflow-hidden mobile-transform" id="user-chat">
            ...
            <div class="chat-content d-lg-flex h-100">
                ...
                <div id="start-screen" class="w-100 overflow-hidden position-relative bo-lf-rt h-100 start-screen" >
                    <div class="d-flex flex-column align-items-center py-5" style="gap: 12px;">
                        <span class="fs-18 mb-5 text-center zero_day_label d-none">Click a channel or participant name on the left to start a conversation.</span>
                        <img class="no-network-place" src="<?php echo TAOH_CDN_MAIN_PREFIX . '/images/empty_network.png';?>" width="300" alt="no-network">
                    </div>
                </div>

                <!-- start chat conversation section -->
                <div id="channel-chat-div" class="w-100 overflow-hidden position-relative bo-lf-rt h-100 d-none" >
                    ...
                    <div id="browse_channels_wrapper" class="middle_block  position-relative" style="display: none; max-width: 1200px;">
                        <div class="px-4 py-3 pb-xl-0">
                            <div class="d-flex justify-content-between d-xl-none">
                                ...
                                <div class="dropdown user-chat-nav" style="display: none;">
                                    <button class="btn nav-btn dropdown-toggle after-none border-0" type="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                        <i class="bx bx-dots-vertical-rounded"></i>
                                    </button>
                                    <div class="dropdown-menu dropdown-menu-end">
                                    ...
                                    <div class="d-flex flex-wrap align-items-center">
                                        <div class="d-flex flex-wrap align-items-center flex-grow-1">
                                            <div class="form-group flex-grow-1 mb-2">
                                                ...
                                                <span class="la la-search input-icon" style="left:unset; right: 15px;"></span>
                                            </div>
                                        </div>
                                    </div>
                                </form>
                    ...
                    <div id="speed_networking" class="middle_block position-relative d-none" style="max-width: 1200px;">
                        <div>
                                        ...
                                        <ul class="list-inline mb-0">
                                            <li class="list-inline-item me-2 ms-0">
                                                <div class="dropdown user-chat-nav d-xl-none">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-end">
                                                        <a class="dropdown-item d-flex justify-content-between align-items-center networking-sidebar-show sn_more_info_btn" href="#">More info <i class="bx bx-info-circle text-muted"></i>
                                                        </a>
                                                    </div>
                                                </div>
                                            </li>
                            ...
                            <div class="loaderArea" style="text-align: center; margin-top: 10px;"></div>
                    ...
                    <!-- conversation user -->
                    <div id="users-chat " class="middle_block  position-relative d-none" data-channel_id="" data-chatwith="" style="">
                        <div class="py-3 user-chat-topbar">
                            <div class="row align-items-center flex-nowrap">
                                <div class="col">
                                    <div class="d-flex align-items-center">
                    ...
                    <div class="d-flex flex-column flex-md-row channel-chat-parent-div">
                    <?php                 
                    if ($isEventNetworking) {
                        //  $watchPartyEnabledChannel = 1;
                    ?>
                        <!-- watch party -->
                        <div class="middle_block  watch-party watchPartySection flex-grow-1 video-section" style="display:none; border-right: 1px solid #d3d3d3;">
                            <div class="card h-100">
                                <div class="card-header wp-head-h" id="headingOne">
                                    <span title="Open Channels Menu" onclick="showLeftSidebar();" id="mini_menu" style="margin-right:20px;"><i class="fa-solid fa-bars"></i></span>
                                ...
                                <div class="card-body video-container d-flex align-items-center">
                                    ...
                                    <div id="watchPartyMeetingLinkDiv" 
                                        style="width: 100%; height: 232px; display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center;">
                    ...
                    <!-- conversation group -->
                    <div id="channel-chat" class="middle_block  position-relative flex-grow-1 watchPartySection" data-channel_id="" data-channel_type="" style="display: none;">
                        <div class="py-3 user-chat-topbar">
                            <div class="row align-items-center flex-nowrap">
                                <div class="col">
                                    <div class="d-flex align-items-center">
                                        <div class="flex-shrink-0 d-block d-xl-none me-3">
                                            <a href="javascript: void(0);" class="btn-primary user-chat-remove user-chat-remove-btn fs-18 p-1">
                                                <i class="bx bx-chevron-left align-middle text-white"></i>
                                            </a>
                                        </div>
                                                ...
                                                <div class="flex-grow-1 overflow-hidden">
                                                    <h6 class="d-flex align-items-center mb-0 fs-18" style="gap: 6px;">
                                                        ...
                                                        <div class="d-flex align-items-center" style="width: 100%;">
                                                            ...
                                                            <div class="flex-grow-1 overflow-hidden">
                                                                <h6 class="text-truncate mb-0 cw_channel_title text-capitalize" style="white-space: initial;"></h6>
                                    ...
                                    <div class="row g-0 align-items-center">
                                        ...
                                        <div class="col-auto">
                                            <div class="chat-input-links ms-md-3">
                                                ...
                                                <div class="links-list-item" data-bs-container=".chat-input-links" data-bs-toggle="popover" data-bs-trigger="focus" data-bs-html="true" data-bs-placement="top"
                                                    data-bs-content="<div class='loader-line'><div class='line'></div><div class='line'></div><div class='line'></div><div class='line'></div><div class='line'></div></div>">
                                                    <button type="button" id="video-btn" class="btn nav-btn chat-video round-video" data-bs-toggle="modal" data-bs-target="#v-channel-room">
                                                        <i class="bx bx-video"></i>
                                                    </button>
                                                </div>
                                            </div>
                    ...
                    <div id="no_dm_block" class="middle_block  position-relative ">
                        <!-- 0 day scenario will come here-->
                        <div class="d-flex flex-column align-items-center py-5" style="gap: 12px;">
                            <span class="fs-18 mb-5">Click "chat" on anyone to start the conversation </span>
                    ...
                    <div id="zero-day-div" class="middle_block  position-relative d-none">
                        <!-- 0 day scenario will come here-->
                        <div class="d-flex flex-column align-items-center py-5" style="gap: 12px;">
                            <span class="fs-18 mb-5">Click "chat" on anyone to start the conversation</span><!-- You are the first on this networking.. Please wait for a while.... -->
                            <img class="no-network-place" src="<?php echo TAOH_CDN_MAIN_PREFIX; ?>/images/empty_network.png" width="300" alt="no-network">
                         </div>
                    </div>
                    <div id="zero-day-channel" class="middle_block  position-relative d-none">
                        <!-- 0 day scenario will come here-->
                        <div class="d-flex flex-column align-items-center py-5" style="gap: 12px;">
                            <span class="fs-18 mb-5">Click any channel to explore more </span>
                            <img class="no-network-place" src="<?php echo TAOH_CDN_MAIN_PREFIX; ?>/images/empty_network.png" width="300" alt="no-network">
                         </div>
                    </div>
                ...
                <div id="chat-reply-sidebar" class="chat-reply-sidebar" >
                    <div class="position-relative h-100">
                        <div class="user-chat-nav">
                            <div class="d-flex w-100">
                                <div class="flex-grow-1">
                                    <button type="button" class="btn nav-btn chat-reply-show mt-2 mb-2 ml-2">
                                        <i class="bx bx-x d-none d-lg-inline"></i>
                                        <i class="bx bx-left-arrow-alt d-inline d-lg-none" id="mob-chat-reply-close-btn"></i>
                                    </button>
                                </div>
                                <!--<div class="flex-shrink-0">
                                    <div class="dropdown">
                ...
                <div id="chat-like-sidebar" class="networking-sidebar chat-like-sidebar">
                    <div class="user-chat-nav">
                        <div class="d-flex w-100">
                            <div class="flex-grow-1">
                ...
                <div id="user-profile-sidebar" class="user-profile-sidebar" style="display: none;">
                    
                </div>
                <!-- end User profile detail sidebar -->

                <!-- start channel members detail sidebar -->
                <div class="networking-sidebar channelData-sidebar" id="channelData-sidebar" style="display: none;">
                        ...
                        <div class="card mb-3 border-0" id="channel_info_block">
                ...
                <div class="networking-sidebar participants-sidebar ovrflw-y" id="participants-sidebar" style="height: 100%; display: none;">
                    <div class="user-chat-nav p-2 d-xl-none">
                    ...
                    <div class="border-con p-3 mb-3 d-none d-xl-block">
                        ...
                        <div class="d-flex align-items-center flex-wrap" style="gap: 12px;">
                            <?php if($keyword != "dm") { ?>
                            <button id="tourbutton" type="button" class="btn bor-btn d-none">Start Application Tour</button>
                            <?php } ?>
                        ...
                        <div class="card my-3 mx-2 d-none" id="activities_block1-main">
                            ...
                            <div class="card-body">
                             ...
                             <div class="video-room-activity-main d-flex align-items-start" style="gap: 12px;">
                                ...
                                <div class="card">
                                    <div class="card-body py-2 px-2">
                                        <ul class="p-0 mb-0" id="activities-list-session"></ul>
        ...
        <div class="modal fade" id="broadcastModal" tabindex="-1" aria-labelledby="broadcastModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header bg-white">
                        <h5 class="modal-title sm-text" id="broadcastModalLabel">Broadcast Message</h5>
                    </div>
                    <div class="modal-body">
                        <div class="d-flex" style="gap: 21px;">
                            <div class="flex-grow-1">
                                <textarea class="form-control mt-3 mb-3" id="broadcast_message" rows="4" placeholder="Type your message here..."></textarea>                                
                                <div class="invalid-feedback">Message cannot be empty</div>
                                <small style="float: right;" class="text-muted mb-3"><span id="char_count">0</span>/250</small>
                                <div class="d-flex align-items-center mt-4" style="gap: 6px;">
                                    <button type="button" class="btn std-btn mt-4" id="broadcast_message_submit">Send</button>
                                    <button type="button" class="btn bor-btn mt-4" data-bs-dismiss="modal">Cancel</button>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <form id="createVideoForm" method="post" action="<?= taoh_site_ajax_url(); ?>">
                            <div class="row">
                                <div class="form-group col-lg-8">
                                    ...
                                    id="video_name" name="video_name"  class="form-control alphanumericInput video_room_name">
                                </div>
                            </div>
...
$is_event_owner = ( $event_owner_ptoken === $my_ptoken || (is_array($owner_org_ptoken) && in_array($my_ptoken, $owner_org_ptoken)));
...
?>
    ...
    <script type="text/javascript">
        ...
        document.addEventListener('DOMContentLoaded', function () {
            new bootstrap.Tooltip(document.body, {
                selector: '[data-bs-toggle="tooltip"]',
                trigger: isMobilePage ? 'click' : 'hover focus',
                html: true,
                sanitize: false
            });

            // Optional: ensure only one tooltip at a time
            document.body.addEventListener('show.bs.tooltip', function () {
                document.querySelectorAll('.tooltip.show').forEach(t => t.remove());
            });

            // Auto hide after 1 second
            document.body.addEventListener('shown.bs.tooltip', function (e) {
        ...
        console.log("ticketInfo ticketInfo", ticketInfo.title);
        ...
        var CURRENT_CHANNEL_TYPE = '';
        let dojoSessionChannelSent = [];
        let dojoSessionChannelScheduled = new Set();

        let joinBtnIntervalId = null;

        const capitalize = str => str.replace(/\b\w/g, c => c.toUpperCase());

        const ticketColors = [
        '#E53935', '#FB8C00', '#1E88E5', '#43A047', '#8E24AA',
        '#3949AB', '#D81B60', '#00897B', '#FDD835', '#6D4C41'
        ];

        const profileColors = [
        '#00ACC1', '#C0CA33', '#FF7043'
        ];
        ...
        function getTicketColor(type) {
            ...
            return ticketColors[Math.abs(hash) % ticketColors.length];
        }

        const profileTypeMap = {}; // runtime mapping

        const isMobilePage = window.matchMedia('(max-width: 767px)').matches;

        function getProfileColor(type) {
        if (!type) return profileColors[0];

        type = type.toString().trim().toLowerCase();

        // Assign next color if new profile type
        if (!(type in profileTypeMap)) {
            const nextIndex =
            Object.keys(profileTypeMap).length % profileColors.length;
            profileTypeMap[type] = nextIndex;
        }

        return profileColors[profileTypeMap[type]];
        }
    ...
    <script type="text/javascript">
        ...
        var isMobile = getMobileDevice();

        const myCollapse = document.getElementById('myChannelCollapse');
        const caretIcon = document.getElementById('myChannelCaret');
        ...
        const Collapse = document.getElementById('channelCollapse');
        const caretIconi = document.getElementById('ChannelCaret');
        Collapse.addEventListener('show.bs.collapse', function () {
            caretIconi.classList.remove('fa-caret-down');
            caretIconi.classList.add('fa-caret-up');
        });
        Collapse.addEventListener('hide.bs.collapse', function () {
            caretIconi.classList.remove('fa-caret-up');
            caretIconi.classList.add('fa-caret-down');
        });
        ...
        $(document).ready(function () {
            ...
            $('#searchUser').on('keyup', function () {
                ...
                let visibleCount = 0;
                $('#dmChannelList li.comm-participants').filter(function () {
                    let name = $(this).attr('data-all-data').toLowerCase();
                    let isMatch = name.includes(value);
                    $(this).toggle(isMatch);
                ...
                if (visibleCount === 0) {
                    if($('.no_user_found_li:visible').length == 0) {
                        $('#noResultMsg').show();
                    }                    
                } else {
                    $('#noResultMsg').hide();
            ...
            $('.participants_refresh, li[role="participant"]').on('click', function () {
                ...
                $('.chat-layout').removeClass('watchPartyEnabled');
            ...
            $('#mob-chat-reply-close-btn').on('click', function () {
                $('#channel-chat-div').removeClass('channel-chat-div-wp-reply');
            });

            $('#broadcast_message_btn').on('click', function () {
                $('#broadcastModal').modal('show');
            });            
            $(document).on('input', '#broadcast_message', function () {
                let max = 250;
                let text = $(this).val();
                if (text.length > max) {
            ...
            $('.sn_more_info_btn').on('click', function () {
                $('#participants-sidebar').addClass('sn-moreinfo-sidebar');
            });
            ...
            $(document).on('click', '#dm_refresh', function() {                
            ...
            $(document).on('click', '#broadcast_message_submit', function () {

                let $btn = $(this);
                let message = $.trim($('#broadcast_message').val());
                ...
                if (message === '') {
                    ...
                    $('#broadcast_message').removeClass('is-invalid');
                }
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        taoh_action: "taoh_ntw_add_broadcast_message",
                        message: message,
                        keyword: ntw_keyword,
                        key: my_pToken
                    },
            ...
            if($('.chat-area-fixed').length > 0) {
                $('.chat-area-fixed').hide();
            }
            ...
            if(isMobile) {
                setTimeout(() => {
                    taoh_set_warning_message("This page does not provide a good experience on mobile devices. For the best experience, please use a desktop or laptop computer.");
        ...
        $(window).on('load', async function() {
            ...
            setTimeout(async function () {
                ...
                try {
                    ...
                    //$("#dmChannelList .channel_btn").first().click();
                    //taoh_load_network_entries();
        ...
        async function syncRoomStamp({roomslug, keyword, my_pToken}) {
            ...
            const orgBroadcastMessageChanged = !oldStamp || oldStamp.org_brodcast_message !== roomStamp?.org_brodcast_message;
            ...
            if (channelChanged) {
                ...
                for (const channelId in freshChannelStamp.response) {
                    ...
                    console.log("skipped here channelInfoExistingValues", channelInfoExisting, channelInfoExistingValues);
                    ...
                    let hasUpdates = Number(channelLastStamp) > Number(channelExisting?.last_fetch_stamp ?? 0);
            ...
            if (sessionChanged && eventToken) {
                ...
                for (const channelId in freshSessionChannelStamp.response) {
                    ...
                    const channelInfoExisting = await IntaoDB.getItem(store, channelInfoKey);                    
                    let channelInfoExistingValues = channelInfoExisting?.values?.channels?.[0]?.members ?? [];
                    console.log("skipped here session channelInfoExistingValues", channelInfoExistingValues);
                    // if (!channelInfoExistingValues.includes(my_pToken)) {
                    //     console.log("skipped here ch");                      
                    //     continue;
                    // }
                    ...
                    console.log("session hasUpdates", hasUpdates, Number(channelLastStamp), Number(channelExisting?.last_update_stamp ?? 0));
                    ...
                    if(hasUpdates) {
                        console.log("session hasupdate", hasUpdates);                        
                        if(currentChannelId && currentChannelId == channelId) {
            ...
            if(orgBroadcastMessageChanged && eventToken && !is_event_owner) {
                loadOrganizerBroadcastMessages(roomStamp?.org_brodcast_message || 0);
            }
        ...
        const tribute = new Tribute({
            ...
            values: async (text, cb) => {
                try {
                    ...
                    if($('#channel-chat').attr('data-channel_type') == taohChannelDm) return cb([]);
        ...
        $(document).on('click', '.openProfileSideBar', async function () {
            ...
            $('#user-profile-sidebar').html(
                '<div class="sidebar-loader">' +
                    '<i class="fa fa-spinner fa-spin mr-3"></i> Loading...' +
                '</div>'
            );
            $('.cw_channel_profile_img').attr('src',"");
            $('.cw_channel_title ').text("");
            ...
            const userLiveStatus = await getUserLiveStatus(chatwith);
            const userInfo = await getUserInfo(chatwith, 'full');

            console.log("userInfo ===========", chatwith, userInfo);

            await updateProfileInfo(userInfo, userLiveStatus);

            $('.dmChannelList li').removeClass('active');
            ...
            $("#channel-chat").attr("data-channel_id", 0);
            $("#channel-chat").attr("data-channel_type", taohChannelDm);

            $('#participants').removeClass('d-block');
            $('#channel-chat').addClass('d-block');
            $('#channel-conversation-list').empty();
            ...
            $('.cw_channel_profile_img').attr('src', avatar_image);
			$('.cw_channel_title').html(`${userInfo.chat_name}`);
            ...
            for (const ptoken of ptokens) {
                ...
                if(ptoken != my_pToken) {
                    ...
                    if (channelElem.length) {
                        ...
                        $(`.sn_listing_item_${ptoken}`).remove();
                        updateSpeedNetworkingCarousel();
                        continue;
                    }
                    ...
                    if($(`.speed_networking_carousel #carousel_item_${ptoken}`).length == 0) {
                        ...
                        if($(`#speedChannelListing li.sn_listing_item_${ptoken}`).length == 0) {                        
                            $('#speedChannelListing').append(`<li class="sn_listing_item sn_listing_item_${ptoken}"  data-chatwith="${ptoken}" data-channel_name="${userInfo.chat_name}">
                                <a class="d-flex align-items-center justify-content-between" href="javascript: void(0);">
                                    <div class="channel_btn flex-grow-1 d-flex align-items-center overflow-hidden" title="${userInfo.chat_name || 'User'}">
                                        <div class="flex-shrink-0 me-2 sm-user-info-icon" data-chatwith="${ptoken}">
                                            <div class="chat-user-img align-self-center ms-0">
                                                <img src="${avatar_image}" class="rounded-circle avatar-xs" alt="user-avatar">
                                            </div>
                                        </div>
                                        <div class="flex-grow-1 overflow-hidden w-hide sm-user-info-icon" data-chatwith="${ptoken}">
                                            <span class="text-truncate mb-0 channel_name text-capitalize">${userInfo.chat_name || 'User'}</span>
                                            <p class="mb-0 fs-12 text-truncate text-secondary fst-italic"></p>
                                        </div>   
                                        <button class="btn btn-outline-primary btn-sm fs-12 connect_btn" data-chatwith="${ptoken}">Connect</button>
                                    </div>
                                </a>
                            </li>`);
                        }
                        ...
                        <div class="d-flex flex-column flex-lg-row" style="gap: 13px;">
                            <div class="mobile-flex">
                                ...
                                <div class="ml-3">
                                    <h5 class="speed-v1-title mb-2 mobile-block-text" style="text-transform: capitalize;">${userInfo.chat_name}</h5>
                                    <p class="speed-v1-badge mb-2 mobile-block-text" style="text-transform: capitalize;">
                            ...
                            <div class="d-flex flex-column justify-content-between" style="min-height: 200px;">
                                <div>
                                    <h5 class="speed-v1-title mb-2 mobile-none-text" style="text-transform: capitalize;">${userInfo.chat_name}</h5>
                                    <p class="speed-v1-badge mb-2 mobile-none-text" style="text-transform: capitalize;">
                                    ${userInfo?.type ? userInfo.type : 'Professional'}
                                    </p>
                                ...
                                <div>
                                    <button type="button" data-chatwith="${ptoken}" class="btn std-btn connect_btn mr-2 mb-2">Connect</button>
                                    <button type="button" data-chatwith="${ptoken}" class="btn bor-btn mb-2 not_interested_btn">Not interested</button>
                                </div>
                            </div>
                        </div>
                        </div>`);
        ...
        $(document).on('click', '#video-btn', function () {
            let currentValue = userInfoChatName+" Video Room";
            if($('#channel-chat').attr('data-channel_type') == taohChannelDm || $('#channel-chat').attr('data-channel_type') == taohChannelOrganizer) {                
                $('.video_room_name').val(currentValue);
            } else {
                let channelTitle = $('.cw_channel_title').text();
                channelTitle = channelTitle.charAt(0).toUpperCase() + channelTitle.slice(1);
                $('.video_room_name').val(channelTitle + ' - ' + currentValue);
            }
            ...
            $.ajax({
                ...
                success: function (response, textStatus, jqXHR) {
                    if (response.success && response.output['mood_status']) {
                        ...
                        taoh_set_success_message("Status updated successfully");
                    } else {
                        update_mood_status('');
                    }
                }
                ...
                if (intao_data && intao_data.timestamp && !((Date.now() - intao_data.timestamp) > 10000) && !serverFetch) {
                    ...
                    $.ajax({
                        ...
                        success: function (response, textStatus, jqXHR) {                                                     
                            ...
                            console.log("=== taoh_load_network_entries =--==", response, response.output.totalcount);

                            if(response && response.output.totalcount == 0) {
                                if($('#pills-direct-tab').hasClass('active')) {
                                    $('.zero_day_label').addClass('d-none');
                                    $('#dmChannelList').html('<li class="text-center no_user_found_li p-3">No users found</li>');
                                ...
                                $('.zero_day_label').removeClass('d-none');
                            }
                            if (response.success && response.output) {
                                ...
                                const shouldRender = (isSearchMode && clientTerm && clientTerm === serverTerm) || (!isSearchMode && !serverTerm);
            ...
            $.ajax({
                ...
                success: function (response, textStatus, jqXHR) {
                    ...
                    if (response && response.success && response.output) {                       
                        ...
                        addParticipantToDmList(response?.output?.items);
                    } else {
        ...
        async function addParticipantToDmList(memberList) {
            console.log("----====== RESPONSES LOAD PART =====---", memberList);
            ...
            for (const [ptoken, entry] of Object.entries(memberList)) {
                ...
                const profileTitle = cell?.type === 'provider' ? 'Service Provider' : capitalize(cell?.type);

                var ticket_type_name = "";
                
                var ticket_details = entry.ticketInfo ?? '';

                if(ticket_details !='' && ticket_details != null && ticket_details != undefined) {
                    ticket_type_name = ticket_details[0].title;
                    console.log("addparticipant", ticket_type_name, ptoken);                    
                    ticketTypeByPtoken[ptoken] = ticket_type_name;
                }

                var companies = roles = company = role = "";

                if(cell.company != null && cell.company != "" && cell.company !== undefined) {
                    companies = formatObject(cell.company);
                    companies = companies[0]?.name;
                    if (isValidValue(companies)) {
                        company = companies;
                    }
                }                   

                if(cell.title != null && cell.title != "" && cell.title !== undefined) {
                    roles = formatObject(cell.title);
                    roles = roles[0]?.name;
                    if (isValidValue(roles)) {
                        role = roles;
                    }
                }      
                
                let rolecompany = "";
                if (isValidValue(role) && isValidValue(company)) {
                    rolecompany = `${role}, ${company}`;
                } else if (isValidValue(role)) {
                    rolecompany = role;
                } else if (isValidValue(company)) {
                    rolecompany = company;
                }
                
                let liTitleParts = [];

                // Profile Type (always shown)
                liTitleParts.push(
                    "<i class='fa-solid fa-circle-user'></i> Profile Type: " + capitalize(profileTitle)
                );

                // Ticket Type (only if exists)
                if (isValidValue(ticket_type_name)) {
                    liTitleParts.push(
                        "<i class='fa-solid fa-ticket'></i> Ticket Type: " + capitalize(ticket_type_name)
                    );
                }
                ...
                if (isValidValue(role)) {
                    liTitleParts.push(
                        "<i class='fa-solid fa-briefcase'></i> Role: " + capitalize(role)
                    );
                }
                ...
                if (isValidValue(company)) {
                    liTitleParts.push(
                        "<i class='fa-solid fa-building'></i> Company: " + capitalize(company)
                    );
                }

                // Location
                if (isValidValue(cell?.full_location)) {
                    liTitleParts.push(
                        "<i class='fa-solid fa-location-dot'></i> Location: " + capitalize(cell.full_location)
                    );
                }

                // Final HTML
                let liTitle = liTitleParts.join("<br>");


                const skillContent = buildSkillContentText(cell.skill, cell.ptoken);  
                const fallbackSrc = `${_taoh_ops_prefix}/avatar/PNG/128/${cell?.avatar?.trim() || 'default'}.png`;
                const userAvatarSrc = await buildAvatarImage(cell.avatar_image, fallbackSrc);
                const dmEntryHtml = `
                    <li class="openProfileSideBar comm-participants profile-tooltip" data-chatwith="${ptoken}" data-all-data="${cell?.chat_name+"~"+cell?.type+"~"+cell?.full_location+"~"+companies+"~"+roles+"~"+skillContent+"~"+ticket_type_name}" data-avatar_src="${userAvatarSrc}" data-channel_type="${taohChannelDm}" data-channel_name="${cell?.chat_name}" data-ticket_type_name="${ticket_type_name}" data-bs-toggle="tooltip" data-bs-title="${liTitle}">
                        <a class="d-flex align-items-center justify-content-between" href="javascript: void(0);">
                            <div class="channel_btn flex-grow-1 d-flex align-items-center overflow-hidden" >
                                <div class="flex-shrink-0 me-2 dm-user-info-icon" data-chatwith="${ptoken}">
                                    <div class="chat-user-img align-self-center ms-0">
                                        <img src="${userAvatarSrc}" class="rounded-circle avatar-xs" alt="user-avatar">
                                    </div>
                                </div>
                                <div class="flex-grow-1 overflow-hidden w-hide dm-user-info-icon chat_name_div" data-chatwith="${ptoken}">
                                    <span class="text-truncate mb-0 chat_name text-capitalize mr-2" >${cell?.chat_name || 'User'}</span>
                                </div>                    
                            </div>
                        </a>
                    </li>
                ...
                var chkliElem = $(`#dmChannelList li[data-chatwith="${ptoken}"]`);
                const chatName = cell?.chat_name || 'User';

                if (!chkliElem.length) {
                    $('#dmChannelList').append(dmEntryHtml);
                } else {
                    chkliElem.find('.chat_name').text(chatName);
                    chkliElem.attr('data-channel_name', chatName);
                }

                var liElem = $(`#dmChannelList li[data-chatwith="${ptoken}"]`);

                const userIcon = `
                    <i class="fa-solid fa-circle-user mt-1 mr-1"
                    data-bs-toggle="tooltip"
                    title="${capitalize(profileTitle)}"
                    style="color:${getProfileColor(profileTitle)};">
                    </i>
                `;

                const ticketIcon = ticket_type_name
                    ? `
                    <i class="fa-solid fa-ticket mr-1 mt-1"
                    data-bs-toggle="tooltip"
                    title="${capitalize(ticket_type_name)}"
                    style="color:${getTicketColor(ticket_type_name)};">
                    </i>
                    `
                    : '';
                ...
                let chatNameHtml = `
                    <div class="chat-name-row">
                        <span class="text-truncate mb-0 chat_name text-capitalize" style="width: 75%;line-height: 14px;">
                            ${chatName}
                        </span>
                ...
                if (ticket_type_name) {
                    chatNameHtml += `
                        <div class="mb-0 fs-12 text-truncate ticket-type-text" style="border-radius:5px; color:#877d7d; display:flex; font-weight: 400;">
                            <span class="chat-role-loc">
                                ${rolecompany}
                            </span>
                        </div>
                    `;
                ...
                if (userMoodStatus?.trim()) {
                    chatNameHtml += `
                        <span class="mb-0 fs-12 text-truncate user-mood-status"
                            title="${stripHtml(userMoodStatus)}"
                            style="display:block; font-style:italic; font-weight: 400;">
                            ${userMoodStatus}
                        </span>
                    `;
                }
                ...
                liElem.find('.chat_name_div').html(chatNameHtml);
                liElem
                    .attr('data-ticket_type_name', ticket_type_name || '')
                    .attr('data-bs-title', `${liTitle}`)
                    .attr(
                        'data-all-data',
                        [
                            chatName,
            ...
            const dm_refreshElem = $('#dm_refresh');
            const dm_refresh_icon = dm_refreshElem.find('i');
            dm_refresh_icon.removeClass('fa-spinner fa-spin').addClass('fa-refresh');
        ...
        function getOrganizerOnlineStatus() {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    const $organizerBlock = $('#pills-org-tab');
                    ...
                    if (isOnline) {
                        ...
                        if($('.chat_with_organizer_div').length > 0) {
                            $('.chat_with_organizer_div').removeClass('d-none').addClass('mt-lg-auto');
                            $('.status_modal_btn').removeClass('mt-lg-auto');    
                        }                    
                    } else {
                        $organizerBlock.addClass('d-none');
                        $('.chat_with_organizer_div').addClass('d-none').removeClass('mt-lg-auto');
                        $('.status_modal_btn').addClass('mt-lg-auto');                        
                    }
        ...
        $(document).on('click', '.sm-user-info-icon', async function () {
            //$('#channel-chat').addClass('d-block');
            //$('.chat-input-bottom').addClass('d-none');

            //$('#channel-chat-div').addClass('d-none');
            //$('.start-screen').removeClass('d-none');

            //$('.channel-item').removeClass('active');

            var chatwith = $(this).attr('data-chatwith');
            $('.sn_listing_item').removeClass('active');

            $('.speed_networking_carousel .carousel-item').removeClass('active');
            $('#carousel_item_'+chatwith).addClass('active');
            $(this).closest('.sn_listing_item').addClass('active');
...
<script>
    ...
    async function saveChannelToDB(channel_id, messages, metadata, update_timestamp = 0) {
        ...
        await IntaoDB.setItem(objStores.ntw_store.name, {
            ...
            last_update_stamp,
            last_noti_read_stamp: last_update_stamp,
    ...
    $(document).on("click", ".view-replies, .btnReply", function () {
        ...
        if ($('.watchPartySection').hasClass('watchPartyEnabled')) {
            $('#channel-chat-div').addClass('channel-chat-div-wp-reply');
        }
    ...
    async function renderChannelList(channelArray = [], tempUserChannelArray = []) {
        ...
        for (const item of list) {
           ...
           // console.log("channelExisting?.last_update_stamp == "+channelId, channelExisting?.last_update_stamp)
    ...
    async function renderSpeedChannelList(channelArray = [], tempUserChannelArray = []) {
        ...
        const snchannelKey = `room_${ntw_keyword}_${roomslug}_${my_pToken}_${taohChannelSpeedNtw}_channel_updates`;
        ...
        //             let toUserInfo = await getUserInfo(ptoken, 'public');
        ...
        //             const dmEntryHtml = `
        //             <li class="channel-item" data-chatwith="${ptoken}" data-channel_name="${toUserInfo?.chat_name}">
        //                 <a class="d-flex align-items-center justify-content-between" href="javascript: void(0);">
        //                     <div class="channel_btn flex-grow-1 d-flex align-items-center overflow-hidden" title="${toUserInfo?.chat_name || 'User'}">
        //                         <div class="flex-shrink-0 me-2">
        //                             <div class="chat-user-img align-self-center ms-0">
        //                                 <img src="${userAvatarSrc}" class="rounded-circle avatar-xs" alt="user-avatar">
        //                             </div>
            ...
            html += `<li id="channel-${channelId}" class="channel-item ${isActive ? 'active' : ''} ${isTempChannel ? 'temp' : ''}" data-channel_passcode="" data-speed_networking="0" data-channel_ticket_slug="" data-name="channel" data-channel_id="${channelId}" data-channel_type="${channelType}" data-channel_name="${channelName}">
                    <a class="d-flex align-items-center justify-content-between" href="javascript: void(0);">
                        <div class="channel_btn flex-grow-1 d-flex align-items-center overflow-hidden" title="${channelName}">
                                ...
                                <div class="chat-user-img align-self-center">
                                    <div class="avatar-xs">
                                        <span class="avatar-title bg-white rounded-circle border text-dark">
                                            ...
                                            <!-- <circle cx="100" cy="100" r="90" fill="none" stroke="#000000" stroke-width="12"/> -->
                                            <path d="M110 30 L110 95 L150 95 L90 170 L90 105 L50 105 Z" fill="#000000" stroke="#000000" stroke-width="12" stroke-linejoin="round" stroke-linecap="round"/>
                                            </svg>
                                        </span>
                                    </div>
                                </div>
                            ...
                            <div class="flex-shrink-0">
                                <div data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Refresh">
                                    <button type="button" class="btn btn-success btn-sm p-2" id="speednetworking_refresh_btn" data-channel_id="${channelId}">
                                        <i class="fa align-middle fa-refresh"></i>
                                    </button>
                                </div>
                            </div>
    ...
    async function renderDMChannelList(channelArray = []) {
        ...
        for (const item of list) {
            ...
            html += `<li id="channel-${channelId}" class="openchatacc comm-participants ${isActive ? 'active' : ''} ${(hasUpdates ==  1) ? 'have_updates' : ''} ${isTempChannel ? 'temp' : ''}" data-channel_passcode="" data-speed_networking="0" data-channel_ticket_slug="" data-name="channel" data-channel_id="${channelId}" data-channel_type="${channelType}" data-avatar_src="${userAvatarSrc}" data-channel_name="${channelName}" data-all-data="" data-chatwith="${to_ptoken}" data-toggle="tooltip" data-bs-toggle="tooltip">
                    <a class="d-flex align-items-center justify-content-between" href="javascript: void(0);">
                        <div class="channel_btn flex-grow-1 d-flex align-items-center overflow-hidden">
                            <div class="flex-shrink-0 me-2">
                                <div class="chat-user-img align-self-center ms-0">
                            ...
                            <div class="flex-grow-1 overflow-hidden w-hide chat_name_div">
                                <span class="text-truncate mb-0 channel_name chat_name text-capitalize">${channelName} </span>
                            </div>
                        </div>
                    </a>
                </li>`;
        ...
        updateUnreadCount('direct');

        if ($('#dmChannelList li.have_updates').length > 0) {
            var dmlist = $("#dmChannelList");
            dmlist.find("li.have_updates").prependTo(dmlist);
        }
    ...
    function updateUnreadCount(type) {
        ...
        setTimeout(function () {
            ...
            if (type === 'channel') {
                count =
                    $('#myChannelList .have_updates').length +
                    $('#channelList .have_updates').length +
                    $('#exhibitorChannelList .have_updates').length +
                    $('#sessionChannelList .have_updates').length;
            } else if (type === 'direct') {
                count = $('#dmChannelList .have_updates').length;
            }
            ...
            if (count > 0) {
                $(`li[role="${type}"] a`).append(
                    `<span class="badge bg-danger fs-11 rounded-pill sidenav-item-badge">${count}</span>`
                );
            }
                        ...
                        <a class="d-flex align-items-center justify-content-between" href="javascript: void(0);">
                            <div class="channel_btn flex-grow-1 d-flex align-items-center justify-content-center overflow-hidden" title="${channelName}">
                                    ...
                                    <div class="chat-user-img online align-self-center">
                                        <div class="avatar-xs">
                                            <span class="avatar-title bg-white rounded-circle border text-white">
                                                <img src="${exhibitorLogoSrc}" class="rounded-circle avatar-xs" alt="user-avatar" onerror="this.onerror=null; this.src='${_taoh_cdn_main_prefix + '/images/empty_network.png'}';">
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="flex-grow-1 align-items-center overflow-hidden w-hide" style="gap: 8px;">
                                    ...
                                    <!-- raffle svg -->
                                    <svg class="" style="min-width: fit-content;display: ${parseInt(exhibitorRaffles, 10) === 1 ? 'block' : 'none'};" width="13" height="13" viewBox="0 0 13 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                                 ...
                                 <div class="d-none watch-enable-no">
                                ...
                                <div class="flex-shrink-0 ms-2 lh-1 d-none">
    ...
    async function checkJoinButtonAvailability() {
        let channel_type = $('#channel-chat').attr('data-channel_type');
        const store = objStores.ntw_store.name;
        const channelInfoKey = `channel_info_${roomslug}_${currentChannelId}_${channel_type}`;        
        ...
        let channelInfoData = channelExisting?.values?.channels?.[0]?.data ?? [];
        var channel_live_status = isJoinEnabled(channelInfoData);

        return channel_live_status;
    }

    function startJoinButtonCheck() {
        if (joinBtnIntervalId) return;

        joinBtnIntervalId = setInterval(async () => {
            let status = await checkJoinButtonAvailability();
            console.log('Meet Join status:', status);
            if(status === false) {
    ...
    function stopJoinButtonCheck() {
        if (joinBtnIntervalId) {
            clearInterval(joinBtnIntervalId);
            joinBtnIntervalId = null;
            console.log('🛑 Join button check stopped');
        }
    }

    async function renderSessionChannelList(channelArray = []) {

        console.log("RENDER_SESSION_CHANNEL_LIST channelArray ---- ", channelArray);        
        ...
        let html = '';
        const store = objStores.ntw_store.name;
        $('.channel-session').remove();
        let activities_html = '';
        var dojoTiming = 5000; //5 seconds
        for (const item of list) {
            var channelId = item.id;
            var channelType = item.type;
            var channelName = item.data?.name || '(unnamed)';
            ...
            if(channelInfo?.data?.spk_state == 'active' || channelInfo?.data?.spk_state == 'live') {
                ...
                if(currentChannelId == channelId) {
                    ...
                    $('.direct-nav .nav-link').addClass('active');
                    $('#channel-chat').removeClass('d-none');
                    $('#pills-direct').addClass('active').addClass('show');
                }                
            }

            var channel_live_status = isJoinEnabled(channelInfo.data);
            var channel_time_gap = getTimeGap(channelInfo.data);  
            
            
            //console.log("event_live_status ===>>",channelName,"==============", channel_live_status);
            //console.log("getTimeGap ===>>",channelName,"==============", channel_time_gap);
            var t_time = Math.floor(channel_time_gap / (1000 * 60));
            if(visibility && channel_live_status){

                //console.log('===kalpana========',channelId);
                if (dojoSessionChannelScheduled.has(channelId)) {
                    console.log('Already scheduled:', channelName);
                    //return;
                }
                else{
                    dojoSessionChannelScheduled.add(channelId);
                    dojoTiming += 5000;
                    const delay = dojoTiming;

                    //console.log(channelName + ' -----2------ ' + delay);

                    const name = channelName;

                    setTimeout(() => {
                        //console.log("===kalpana========" + name + " is live now!");
                        taoh_show_queued_success(name + " is live now!");
                        // mark as finally sent
                        dojoSessionChannelSent.push(channelId);
                    }, delay);
                }

                var activities_html_a = `<li id="channel-session-${channelId}" class="channel-session d-flex align-items-center activity-list" style="gap: 12px;">
                                                
                                                <div>
                                                    <p class="fs-12 fw-400 mb-0" style="line-height: 1.149;">
                                                    <span class="fw-500 text-capitalize text-underline activity-open-channel" id="${channelId}">
                                                    ${channelName}
                                                    </span> is live now!</p>
                                                    
                                                </div>
                                            </li>`;
                    $('#activities-list-session').append(activities_html_a);
            }
            else if(t_time < 30 && channel_time_gap > 0){
                    var activities_html_a = `<li id="channel-session-${channelId}" class="channel-session d-flex align-items-center activity-list" style="gap: 12px;">
                                                ...
                                                <div>
                                                    ...
                                                    <span class="fw-500 text-capitalize text-underline activity-open-channel" id="${channelId}">
                                                    ${channelName}
                                                    </span> is going to start in another ${t_time} mins</p>
                            ...
                            <div class="channel_btn flex-grow-1 d-flex align-items-center justify-content-center overflow-hidden" title="${channelName}">
                                <div class="flex-shrink-0 me-2">
                                    <div class="chat-user-img online align-self-center">
                                        <div class="avatar-xs">
                                            <span class="avatar-title bg-white rounded-circle border text-white">
                                                <img src="${sessionLogoSrc}" class="rounded-circle avatar-xs" alt="user-avatar" onerror="this.onerror=null; this.src='${_taoh_cdn_main_prefix + '/images/empty_network.png'}';">
                                            </span>
                                        </div>
                                    </div>
                                </div>
    ...
    async function loadOrganizerBroadcastMessages(timestamp) {
        $.ajax({
            url: _taoh_site_ajax_url,
            type: "post",
            data: {
                roomslug: ntw_room_key,
                taoh_action: "taoh_ntw_get_broadcast_message",
                keyword: ntw_keyword,
                key: my_pToken,
                timestamp,
            },
            dataType: "json",
            success: function (res) {         
                console.log("taoh_ntw_get_broadcast_message", res);              
                if (res.success === true) {                    
                    if (Array.isArray(res.messages) && res.messages.length > 0) {
                        const firstMessage = res.messages[0].message;
                        const firstTimestamp = res.messages[0].timestamp;
                        if(firstMessage) {
                            taoh_set_success_message(firstMessage, 10000);
                        }                        
                    }
                } else {
                    alert(res.message || 'Failed to send broadcast message');
        ...
        //setTimeout(() => {
            ...
            if (channelElem.length) {
                ...
                if (firstChannelButton.length) {
                        if(chatwithchanneltype != taohChannelDm) {
                            $('.tab-pane').removeClass('show').removeClass('active');
                            $('.nav-link').removeClass('active');
                            $('.channel-nav .nav-link').addClass('active');
                            $('#pills-channel').addClass('active').addClass('show');                            
                        }
    ...
    function send_message(channel_id, channel_type, message_text, is_background_send = 0) {
        ...
        var chatwith_user = $("#channel-chat").attr("data-chatwith");
    ...
    async function load_channel_messages(channel_id) {
        ...
        //let last_read_timestamp = channelData?.last_read_stamp || 0;
    ...
    async function saveChannelMessages(channel_id, render = 0, channel_type = '') {
        ...
        const channelInfoKey = `channel_info_${roomslug}_${channel_id}_${channel_type}`;
        const channelInfoExisting = await IntaoDB.getItem(objStores.ntw_store.name, channelInfoKey);                    
        let channelInfoExistingValues = channelInfoExisting?.values?.channels?.[0]?.members ?? [];
        console.log("skipped here channelInfoExistingValues", channelInfoExisting, channelInfoExistingValues);                    
                ...
                success: async function (res) {
                    try {
                        if(Array.isArray(res) && res.length > 0) {
                            ...
                            if (res[0].text && res[0]?.ptoken !== my_pToken) {
                                const anchor = getAnchorFromHtml(res[0].text);
                                if (anchor) {
                                    const videoName = anchor.textContent.trim();
                                    if(anchor && anchor.classList.contains("join-v-link")) {
                                        if (channel_type === taohChannelDm) {
                                            if(channelInfoExistingValues.includes(my_pToken) ) {
                                                taoh_set_success_message(`${videoName} just started 📡<br>Join here ${anchor}`, 10000);
                                            }
                                        } else {
                                            taoh_set_success_message(`${videoName} just started 📡<br>Join here ${anchor}`, 10000);
                                        } 
    ...
    function getAnchorFromHtml(html) {
        if (!html) return null;

        const tempDiv = document.createElement("div");
        tempDiv.innerHTML = html;

        return tempDiv.querySelector("a.join-v-link");
    }

    async function loadSpeedNetworkingAllData(channel_id) {
        var lastTimestamp = 0;
        const response = await new Promise((resolve, reject) => {
            $.ajax({
                url: _taoh_site_ajax_url,
        ...
        if (response.status == 200) {
            $('#speedChannelListing').html("");
            //$('#speedChannelRejectList').html("");
            ...
            if (Array.isArray(res.myaccept_ptoken) && res.myaccept_ptoken.length > 0) {
                for (const ptoken of res.myaccept_ptoken) {
                    //const [accept_ptoken, video_link] = Object.entries(item)[0];

                    const user = await getUserInfo(ptoken, 'public');
                    if (user) {
                        const fallbackAvatar = `${_taoh_ops_prefix}/avatar/PNG/128/${user.avatar || 'default'}.png`;
                        const avatarSrc = await buildAvatarImage(user.avatar_image, fallbackAvatar);

                        if($(`#speedChannelListing li.accepted_item_${ptoken}`).length == 0) {
                            $('#speedChannelListing').append(`
                            <li class="accepted_item_${user.ptoken}" data-chatwith="${user.ptoken}" data-channel_name="${user.chat_name}">
                                <a class="d-flex align-items-center justify-content-between" href="javascript: void(0);">
                                    <div class="channel_btn flex-grow-1 d-flex align-items-center overflow-hidden" title="${user.chat_name || 'User'}">
                                        <div class="flex-shrink-0 me-2">
                                            <div class="chat-user-img align-self-center ms-0">
                                                <img src="${avatarSrc}" class="rounded-circle avatar-xs" alt="user-avatar">
                                            </div>
                                        </div>
                                        <div class="flex-grow-1 overflow-hidden w-hide">
                                            <span class="text-truncate mb-0 channel_name text-capitalize">${user.chat_name || 'User'}</span>
                                            <p class="mb-0 fs-12 text-truncate text-secondary fst-italic">Connected</p>
                                        </div>                      
                                    </div>
                                </a>
                            </li>`);
                    ...
                    // console.log("GET speed channel data =--=", accept_ptoken);
                    // $('#speedChannelAcceptList').append(`
                    //     <li id="speed_networking_accept_ptoken_${accept_ptoken}" class="list-group-item d-flex justify-content-between align-items-center" data-ptoken="${accept_ptoken}">
                    //         <span>${accept_ptoken}</span>
                    //         <a href="${video_link}" target="_blank" class="btn btn-sm btn-primary">Join Video</a>
                    //     </li>
                    // `);
                }
            }
            //request_ptoken
            if (Array.isArray(res.myrequest_ptoken) && res.myrequest_ptoken.length > 0) {
                for (const ptoken of res.myrequest_ptoken) {
                    const user = await getUserInfo(ptoken, 'public');
                    if (user) {
                        const fallbackAvatar = `${_taoh_ops_prefix}/avatar/PNG/128/${user.avatar || 'default'}.png`;
                        const avatarSrc = await buildAvatarImage(user.avatar_image, fallbackAvatar);

                        if($(`#speedChannelListing li.requested_item_${ptoken}`).length == 0) {
                            $('#speedChannelListing').append(`
                            <li class="requested_item_${user.ptoken} sm-user-info-icon" data-chatwith="${ptoken}" data-channel_name="${user.chat_name}">
                                <a class="d-flex align-items-center justify-content-between" href="javascript: void(0);">
                                    <div class="channel_btn flex-grow-1 d-flex align-items-center overflow-hidden" title="${user.chat_name || 'User'}">
                                        <div class="flex-shrink-0 me-2">
                                            <div class="chat-user-img align-self-center ms-0">
            ...
            if (Array.isArray(res.request_ptoken) && res.request_ptoken.length > 0) {
                for (const ptoken of res.request_ptoken) {
                    const user = await getUserInfo(ptoken, 'public');
                    if (user) {
                        const fallbackAvatar = `${_taoh_ops_prefix}/avatar/PNG/128/${user.avatar || 'default'}.png`;
                        const avatarSrc = await buildAvatarImage(user.avatar_image, fallbackAvatar);

                        if($(`#speedChannelListing li.incoming_request_item_${ptoken}`).length == 0) {
                            $('#speedChannelListing').append(`
                            <li class="incoming_request_item_${user.ptoken}" data-chatwith="${user.ptoken}" data-channel_name="${user.chat_name}">
                                <a class="d-flex align-items-center justify-content-between" href="javascript: void(0);">
                                    <div class="channel_btn flex-grow-1 d-flex align-items-center overflow-hidden" title="${user.chat_name || 'User'}">
                                        <div class="flex-shrink-0 me-2 sm-user-info-icon" data-chatwith="${user.ptoken}">
                                            <div class="chat-user-img align-self-center ms-0">
                                                <img src="${avatarSrc}" class="rounded-circle avatar-xs" alt="user-avatar">
                                            </div>
                                        </div>
                                        <div class="flex-grow-1 overflow-hidden w-hide sm-user-info-icon" data-chatwith="${user.ptoken}">
                                            <span class="text-truncate mb-0 channel_name text-capitalize">${user.chat_name || 'User'}</span>
            ...
            if (Array.isArray(res.myreject_ptoken) && res.myreject_ptoken.length > 0) {
                for (const ptoken of res.myreject_ptoken) {
                    const user = await getUserInfo(ptoken, 'public');
                    if (user) {
                        const fallbackAvatar = `${_taoh_ops_prefix}/avatar/PNG/128/${user.avatar || 'default'}.png`;
                        const avatarSrc = await buildAvatarImage(user.avatar_image, fallbackAvatar);

                        // if($(`#speedChannelListing li.rejected_item_${ptoken}`).length == 0) {
                        //     $('#speedChannelListing').append(`
                        //     <li class="rejected_item_${user.ptoken}" data-chatwith="${user.ptoken}" data-channel_name="${user.chat_name}">
                        //         <a class="d-flex align-items-center justify-content-between" href="javascript: void(0);">
                        //             <div class="channel_btn flex-grow-1 d-flex align-items-center overflow-hidden" title="${user.chat_name || 'User'}">
                        //                 <div class="flex-shrink-0 me-2">
                        //                     <div class="chat-user-img align-self-center ms-0">
    ...
    async function loadSpeedNetworkingData(channel_id) {
       ...
       // console.log("getChannelTimestamp ===========", lastTimestamp);
        ...
        if (response.status == 200) {
            ...
            console.log("res_timestamp, lastTimestamp, diffInSeconds", res_timestamp, Date.now(), diffInSeconds);
            ...
            if($('.speed_networking_div:visible').length > 0) {
            ...
            if (Array.isArray(res.accept_ptoken) && res.accept_ptoken.length > 0) {
                ...
                for (const item of res.accept_ptoken) {
                    ...
                    if(accept_ptoken != my_pToken) {
                        ...
                        $(`.sn_listing_item_${accept_ptoken}`).remove();
            ...
            if (Array.isArray(res.request_ptoken) && res.request_ptoken.length > 0) {
                //if(speed_networking_last_timestamp != 0) {
                    ...
                    if (userInfo && !$('#connectModal').hasClass('show')) {
                        ...
                        $('#connectModal .accept_btn').attr('data-chatwith', myptoken).attr('data-chatfrom', req_ptoken).attr('data-action', 1).attr('data-chatfrom-name', pinned_by).prop('disabled', false).text('Accept');
                        $('#connectModal .reject_btn').attr('data-chatwith', myptoken).attr('data-chatfrom', req_ptoken).attr('data-action', -1).attr('data-chatfrom-name', pinned_by).prop('disabled', false).text('Ignore');
                ...
                if ($('.countDownDiv:visible').length) {
                    for (const rej_ptoken of res.reject_ptoken) {
                        ...
                        if(rej_ptoken == $('.countDownDiv').data('chatwith')) {
                            ...
                            $('#carousel_item_'+rej_ptoken).remove();
                            $(`.sn_listing_item_${rej_ptoken}`).remove();
    ...
    async function renderVideoActivities(videoactivities) {
        ...
        if (videoactivities.length > 0) {
            ...
            for (let activity of videoactivities) {
                ...
                const channelName = activity.channel_name != undefined && activity.channel_name !== null && activity.channel_name !== '' && activity.channel_name != 'undefined' ? 'on ' + activity.channel_name : '';
                ...
                if (activity.action == 'create_video') {
                    ...
                    activities_video_html += `
                        ...
                        <div>
                            ...
                            created a video room  <b>${activity.video_name}</b> ${channelName}
                        </span>
                    ...
                    activities_html += `<li class="d-flex align-items-center activity-list" style="gap: 12px;">
                                                ...
                                                <div>
                                                    ...
                                                    <div class="d-flex align-items-center" style="gap: 6px;">
                                                        ...
                                                        data-channel_type="${taohChannelDm}"
                                                        data-avatar_src="${userAvatarSrc}"
                                                        data-channel_name="${userChatName}"
                                                        >Chat</a>
                    ...
                    activities_html += `<li class="d-flex align-items-center activity-list" style="gap: 12px;">
                        ...
                        <div>
                            ...
                            <div class="d-flex align-items-center" style="gap: 6px;">
                                ...
                                <a href="#" class="openchatacc a-link  capitalize-first"
                                data-chatwith="${activity.ptoken}"
                                data-chatname="${userChatName}" data-channel_type="${taohChannelDm}" data-avatar_src="${userAvatarSrc}" data-channel_name="${userChatName}">Chat</a>
                            </div>
    ...
    $(document).on('click', '.activity-open-channel', function(e) {
        e.preventDefault();
        let channelId = $(this).attr('id');
        let channelType = $('#channel-'+channelId).attr('data-channel-type');
    ...
    $(document).on('click', '.sn_open_chat_btn', async function () {
        var chatwith = $(this).attr('data-chatwith');
        $('.tab-pane').removeClass('show').removeClass('active');
        $('.nav-link').removeClass('active');
        $('.direct-nav .nav-link').addClass('active');
    ...
    $(document).on('click', '.accept_btn, .reject_btn', async function () {
        ...
        $('.tab-pane').removeClass('show').removeClass('active');
        $('.nav-link').removeClass('active');
        $('.speed-nav .nav-link').addClass('active');
        $('#speed_networking').removeClass('d-none');
        $('#pills-speed').addClass('active').addClass('show');

        $('.sn_channel_title').text("Speed Networking");
        
        var channelId = $('#speedChannelList li:first').attr('data-channel_id');
        var chatWith = $(this).attr('data-chatwith');
        var chatFrom = $(this).attr('data-chatfrom');
        var action = $(this).attr('data-action');
        var chatFromName = $(this).attr('data-chatfrom-name');    
        ...
        var video_name  = "";
        if(action == 1) {
            var sortedNames = [userInfoChatName, chatFromName].sort();
    ...
    async function loadConversation(channel_name, channelType, avatar_src){
        ...
        if ($('#members_block #members_list').length === 0) {
            ...
            <div class="p-3 border-bottom">
                <div class="pb-4 border-bottom border-bottom-dashed mb-4">
                    ...
                    <ul id="members_list" class="list-unstyled chat-list mx-n2">
                    </ul>
                </div>
            </div>`);
        }
        ...
        if (channelInfo.members != undefined && channelInfo.members.length > 0) {
            $.each(channelInfo.members, async function (mkey, memtoken) {
                ...
                if ($('#members_list').find(`#member_${memtoken}`).length === 0) {
                    var members = `
                        <li id="member_${memtoken}" data-chatwith="${memtoken}" class="${(memtoken != "Organizer" && memtoken != my_pToken) ? 'openchatacc' : ''}">
                            <a href="javascript: void(0);">
                                <div class="d-flex align-items-center">
    ...
    async function triggerDMChannel(channel_name, channel_type, avatar_src) {
        $('#zero-day-div').addClass('d-none');

        console.log("triggerDMChannel 1");
       ...
       // $('.start-screen').addClass('d-none');
        ...
        $('.watchPartySection').hide();
        $('.watchPartySection').removeClass('watchPartyEnabled');
        $('.chat-leftsidebar').removeClass('watchPartyEnabled').removeClass('hideSideBar');        
        $('.chat-layout').removeClass('watchPartyEnabled').removeClass('hideSideBar');
        $('#mini_menu').hide(); 
        $('#chat_input').val("");
        $('.pin-message-v2-dm').hide();
        console.log("DMCHANNELLIST CLICKED");
        $('.watchPartySection').removeClass('watchPartyEnabled');
        
        await loadConversation(channel_name, channel_type, avatar_src);
    }
    ...
    $(document).on("click", ".organizerChannelList li", async function() {
        ...
        $('#mini_menu').hide(); 
        const currentElem = $(this);
        const channel_type = currentElem.getSyncedData('channel_type');
        const channel_name = currentElem.getSyncedData('channel_name');
        const avatar_src = currentElem.getSyncedData('avatar_src');
    ...
    async function updateProfileInfo(userInfo, userLiveStatus, fromParticipants = 0) {
        ...
        var companyValue = Object.values(userInfo?.company || {})[0]?.value || "";
        var titleValue = Object.values(userInfo?.title || {})[0]?.value || "";
        var profileTitle = userInfo?.type === 'provider' ? 'Service Provider' : capitalize(userInfo?.type);

        console.log("ticketTypeByPtoken ticketTypeByPtoken", ticketTypeByPtoken, userInfo.ptoken);        
                    ...
                    <div class="user-profile-img">
                        <div class="p-v1-header">
                            ...
                            <div class="d-flex align-items-center" style="gap: 8px;">
                                ...
                                ${profileTitle ? `<span class="p-type text-capitalize"><i class="fa-solid fa-circle-user"></i> ${profileTitle}</span>` : ''}                   
                            </div>
                            ...
                            ${ticketTypeByPtoken[userInfo.taoh_ptoken] ? `<p class="fs-14 text-truncate user-profile-status mt-1 mb-0"><i class="fa-solid fa-ticket"></i> ${ticketTypeByPtoken[userInfo.taoh_ptoken]}</p>` : ''}
            ...
            let rolecompany = rolecompanylabel = "";
            if (isValidValue(titleValue) && isValidValue(companyValue)) {
                rolecompany = `${titleValue}, ${companyValue}`;
                rolecompanylabel = "Role, Company";
            } else if (isValidValue(titleValue)) {
                rolecompany = titleValue;
                rolecompanylabel = "Role";
            } else if (isValidValue(companyValue)) {
                rolecompany = companyValue;
                rolecompanylabel = "Company";
            }

            userProfileData += `${(rolecompany != "") ? `<div class="p-v1-card">
                    <h5 class="p-v1-title">
                        <i class="fa-solid fa-briefcase"></i> 
                        <span>${rolecompanylabel}</span>
                    </h5>
                    <p class="p-v1-content">${rolecompany}</p>
                </div>` : ''}`;
    ...
    async function openChannel(channelId, channelType) {
       ...
       $('#channel-chat').removeClass('d-none');
        ...
        console.log("channel_streaming_link ========", channel_streaming_link);
        

        $('.view_details_link').remove();      
        var view_details_link = '';  
        if(channelInfo?.data?.eventtoken && channelInfo?.data?.exhibitor_id) {
            view_details_link = `${_taoh_site_url_root}/events/exhibitors/${channelInfo.data.exhibitor_id}/${channelInfo.data.eventtoken}`;
            $('.cw_channel_title').append(`<a class="view_details_link" target="_blank" href="${view_details_link}"><i class="ml-2 text-primary fa fa-external-link"></i></a>`);
        }
        if(channelInfo?.data?.eventtoken && channelInfo?.data?.speaker_id) {
            view_details_link = `${_taoh_site_url_root}/events/speaker/${channelInfo.data.eventtoken}/${channelInfo.data.speaker_id}`;
            $('.cw_channel_title').append(`<a class="view_details_link" target="_blank" href="${view_details_link}"><i class="ml-2 text-primary fa fa-external-link"></i></a>`);
        }
        ...
        if(channelInfo?.data?.spk_datefrom && channelInfo?.data?.spk_dateto && channelInfo?.data?.spk_timezoneSelect) {
            ...
            //time_gap = getTimeGap(channelInfo.data);   
            
            console.log("event_live_status ===>>", join_status);
            //console.log("getTimeGap ===>>", time_gap);
        ...
        if(channel_streaming_link) {
            ...
            $('.chat-layout').addClass('watchPartyEnabled');
            $('.chat-leftsidebar').removeClass('hideSideBar');
            $('.chat-layout').removeClass('hideSideBar');
            $('#channel-chat-div').addClass('channel-chat-div-wp');
            $('.channel-chat-parent-div').addClass('video-section-open-block');
            if(!isMobile){
                $('#mini_menu').show(); 
            }
            startJoinButtonCheck();
            ...
            $('.channel-chat-parent-div').removeClass('video-section-open-block');
            $('.animated-menu').removeClass('d-none').addClass('d-sm-block');
            $('.watchPartySection').hide();
            $('.watchPartySection').removeClass('watchPartyEnabled');
            $('#channel-chat-div').removeClass('channel-chat-div-wp');
            if(channelType == taohChannelOrganizer) {
                $('.user-chat-remove-btn').hide();
                $('.chat-leftsidebar').addClass('hideSideBar');
                $('.chat-layout').addClass('hideSideBar');
            } else {
                $('.user-chat-remove-btn').show();
                $('.chat-leftsidebar').removeClass('hideSideBar');
                $('.chat-layout').removeClass('hideSideBar');
            }
            if(!isMobile){
            ...
            if($('#channel-chat:visible').length > 0 && channelType != taohChannelOrganizer) {
                loadRightSidebar('members');
                $('.animated-menu').addClass('d-none').removeClass('d-sm-block');
            }        
            stopJoinButtonCheck();  
        }
        ...
        if ((channelType == taohChannelExhibitor || channelType == taohChannelSession) && channelInfo?.data?.description) {
            const decodedDesc = taoh_desc_decode(channelInfo.data.description);
            const shortDesc = limitText(decodedDesc, 500);
            $('#channel_info_block').html(`<div class="border-con p-3 mb-3 d-xl-block">
                <div class="d-flex">
                    <div class="flex-grow-1">
                        <h5 class="fs-16 text-muted text-uppercase">About</h5>
                    </div>
                </div>
                <div class="d-flex align-items-center flex-wrap mt-2" style="gap: 12px;">
                    ${taoh_desc_decode(shortDesc)}                   
                </div>
                <div class="d-flex align-items-center flex-wrap mt-2" style="gap: 12px; float: right;">
                    ${view_details_link ? `<a target="_blank" href="${view_details_link}" class="text-primary">View more</a>` : ''}
                </div>
            </div>`);
        } else {
            $('#channel_info_block').html('');
        }
        ...
        if (channelMembers != undefined && channelMembers.length > 0) {
            ...
            async function renderMembers(limit) {
                ...
                for (const memtoken of sliceMembers) {
                    ...
                    if ($('#members_list').find(`#member_${memtoken}`).length === 0) {
                        var members = `
                            <li id="member_${memtoken}" data-chatwith="${memtoken}" class="${(memtoken != "Organizer" && memtoken != my_pToken) ? 'openchatacc' : ''}" data-channel_type="${taohChannelDm}" data-avatar_src="${avatar_image}" data-channel_name="${d_data.chat_name}">
                                <a href="javascript:void(0);">
                                    <div class="d-flex align-items-center">
                                        <img src="${avatar_image}" alt="" class="avatar-sm rounded-circle me-3">
                                        <div class="flex-grow-1 overflow-hidden">
                                        ...
                                        <div>
                                            ...
                                            <button type="button" id="${memtoken}" 
                                                class="btn btn-sm mr-2 ${(memtoken == "Organizer") ? 'd-none' : 'ch_member_open_chat_btn'}" 
    ...
    $(document).on("click", ".ch_member_open_chat_btn", function() {
        $('.tab-pane').removeClass('show').removeClass('active');
        $('.nav-link').removeClass('active');
        $('.direct-nav .nav-link').addClass('active');
        $('#channel-chat').removeClass('d-none');
    ...
    async function loadDmWindow(to_ptoken, openVideoRoom = 0, videolink = "", videoname = "", open_dm = true) {
        ...
        $(`.sn_listing_item_${to_ptoken}`).remove();
    ...
    $(document).on('click', '.openchatacc', async function () {
        ...
        console.log("openchatacc_li ---- ");        

        const currentElem = $(this);   
        currentElem.removeClass('have_updates');

        if ($(this).parent().hasClass('dmChannelList')) {
            $('.dmChannelList li').removeClass('active');
            $(this).addClass('active');
        }
        ...
        console.log("openchatacc ---- ", currentChannelId, to_ptoken, input);
        ...
        if (channelElem.length) {
            ...
            await triggerDMChannel(channel_name, channel_type, avatar_src);
            $('#chat-input-container').show();
            $('.chat-input-bottom').removeClass('d-none');
        } else {

            //const chatButtonIconElem = currentElem.find('div.channel_btn');
            currentElem.html('<div class="text-center"><span class="spinner-border spinner-border-sm text-primary"></span></div>');
    ...
    $('#channel_refresh').on('click', async function () {
    ...
    $("#chat-send-btn").on("click", async function() {
        ...
        if(channelType == taohChannelDm) {
            ...
            let cw_channel_title = $('.cw_channel_title').text().trim();
            let cw_channel_profile_img = $('.cw_channel_profile_img').text().trim();

            if(channelId == 0 ) {
                const input = [roomslug, my_pToken, chatwith].sort().join('_');
                channelId = await generateSecureSlug(input, 16);                
                currentChannelId = channelId;
                await createDMChannel(channelId, roomslug, my_pToken, chatwith, cw_channel_title, taohChannelDm, cw_channel_profile_img);
                $("#channel-chat").attr("data-channel_id", channelId);
            }
    ...
    async function createDMChannel(channelId, roomslug, my_pToken, to_ptoken, channel_name, channel_type, avatar_src) {
    ...
    function updateSpeedNetworkingCarousel() {
        ...
        if($('.speedChannelListing li').length > 0) {
            $('.chat-layout').removeClass('hideSideBar');
            $('.chat-leftsidebar').removeClass('hideSideBar');
        } else {
            $('.chat-layout').addClass('hideSideBar');
    ...
    document.addEventListener('click', function (e) {        
        ...
        if (link && link.parentElement) {
            ...
            const roleValue = link.parentElement.getAttribute('role');
            $('.chat-layout').removeClass('watchPartyEnabled');             
            $('.chat-leftsidebar').removeClass('hideSideBar');
            $('.chat-layout').removeClass('hideSideBar');             
            
            if (link && roleValue != "presentation" && link.parentElement.classList.contains('nav-item')) {
                $('#user-chat').removeClass('user-chat-show');
                if(roleValue !='' && roleValue != null && roleValue != undefined) {                

                    $('.chat-leftsidebar').removeClass('speed');
                    $('.chat-leftsidebar').removeClass('channel');
                    $('.chat-leftsidebar').removeClass('direct');
                    $('.chat-leftsidebar').removeClass('org');
                    $('.chat-leftsidebar').removeClass('watchPartyEnabled');
                    ...
                    if(roleValue == 'channel') {
                        ...
                        if(CURRENT_CHANNEL_ID == '' || CURRENT_CHANNEL_ID == null || CURRENT_CHANNEL_ID == undefined || CURRENT_CHANNEL_TYPE == taohChannelOrganizer) {
                            ...
                            if(!$('#channel-chat').hasClass('watchPartyEnabled')) {
                                setTimeout(() => {
                                    $('#zero-day-channel').addClass('d-none');
                                    $('#zero-day-div').addClass('d-none');
                                    //console.log('already channel clicked-----------'+CURRENT_CHANNEL_ID)
                                    //$('#channel-'+CURRENT_CHANNEL_ID).click();
                        ...
                        $('#zero-day-div').addClass('d-none');
                        $('#chat-input-container').removeClass('d-none') ;
                        var org_channel = $('.chat_with_organizer').attr('data-channel_id');
                        const channelType = taohChannelOrganizer;
                        $('#channel-chat').removeClass('d-none');
                        $('#channel-chat').addClass('d-block');

                        openChannel(org_channel, channelType);
                    }
                    else if(roleValue == 'speed') {
    ...
    function showLeftSidebar(){
        if($('.chat-leftsidebar').hasClass('watchPartyEnabled')){
            $('.chat-leftsidebar').removeClass('watchPartyEnabled')
            $('.chat-layout').removeClass('watchPartyEnabled');
        } else {
            $('.chat-leftsidebar').addClass('watchPartyEnabled')
