    ...
    <div class="bg-white">
        <header class="networking-mobile-sticky sticky-top bg-white border-bottom border-bottom-gray">
            <section class="hero-area bg-white shadow-sm overflow-hidden">
                ...
                <div class="container" style="margin-left:0;padding-left:20px;">
                    <?php
    ...
    <div class="layout-wrapper chat-layout d-lg-flex bg-white <?=(!$eventtoken) ? 'non-event': '' ?>" data-bs-theme="light">
        ...
        <!-- start leftsidebar-menu -->
                ...
                <div class="flex-lg-column my-0 sidemenu-navigation">
                    <ul class="nav nav-pills side-menu-nav" role="tablist">
                        ...
                        <li class="nav-item d-lg-block" role="direct">
                        ...
                        <li class="nav-item d-lg-block" role="speed">
                        ...
                        <li class="nav-item d-lg-block" role="channel">
                            ...
                            <div data-intro="" data-step="5" class="d-none1 w-hide">
                                <div class="left-section chat-message-list mb-3">
                                    <div class="collapse show" id="speedChannelCollapse">			
                                        ...
                                        <ul class="speedChannelAcceptList list-unstyled chat-list chat-user-list" id="speedChannelAcceptList"></ul>
                                        <ul class="speedChannelRejectList list-unstyled chat-list chat-user-list" id="speedChannelRejectList"></ul>
                                    </div>
                                </div>
                            </div>
                </div>
                    ...
                    <div class="d-flex align-items-center m-2">
                        <div class="d-flex align-items-center flex-grow-1">
                            ...
                            <div class="search-btn-box mb-2">
                                <button class="btn search theme-btn" id="search" type="submit">
        ...
        <div class="user-chat w-100 overflow-hidden mobile-transform" id="user-chat">
            ...
            <div class="chat-content d-lg-flex h-100">
                ...
                <div class="w-100 overflow-hidden position-relative bo-lf-rt h-100 start-screen">
                ...
                <div class="w-100 overflow-hidden position-relative bo-lf-rt h-100 d-none" id="channel-chat-div">
...
<script>
                    ...
                    try {
                        if(Array.isArray(res) && res.length > 0) {
                            ...
                            if (res.length == 1 && messages.length == 0 && res[0]?.ptoken != my_pToken && channel_type == taohChannelDm) { 
                                ...
                                taoh_set_success_message("You’ve got a new message!");                                
                            }
    ...
    async function loadSpeedNetworkingData(channel_id) {
       ...
       // console.log("getChannelTimestamp ===========", lastTimestamp);
        ...
        if (response.status == 200) {
            ...
            let snchannelData = await IntaoDB.getItem(objStores.ntw_store.name, snchannelKey);
            let snupdatedResponse = snchannelData?.values || [];
            console.log("SPEED N/W GET0", snchannelKey, snupdatedResponse); 
            if (snupdatedResponse) {

                console.log("SPEED N/W GET Start", snchannelKey, snupdatedResponse);
                
                if (!Array.isArray(snupdatedResponse.members)) {
                    snupdatedResponse.members = [];
                }
                if (!Array.isArray(snupdatedResponse.accept_ptoken)) {
                    snupdatedResponse.accept_ptoken = [];
                }
                if (!Array.isArray(snupdatedResponse.reject_ptoken)) {
                    snupdatedResponse.reject_ptoken = [];
                }
                
                if (Array.isArray(res.all_ptoken) && res.all_ptoken.length > 0) {
                    res.all_ptoken.forEach((ptoken) => {
                        snupdatedResponse.members.push(ptoken);
                    });
                }
                if (Array.isArray(res.accept_ptoken) && res.accept_ptoken.length > 0) {
                    console.log("SPEED N/W GET1", snchannelKey, res.accept_ptoken);
                    for (const item_a of res.accept_ptoken) {
                        console.log("SPEED N/W GET2", snchannelKey, item_a);
                        const [accept_ptoken_a, video_link_a] = Object.entries(item_a)[0];
                        console.log("SPEED N/W GET2", snchannelKey, accept_ptoken_a);
                        snupdatedResponse.accept_ptoken.push(accept_ptoken_a);
                    }
                }
                if (Array.isArray(res.reject_ptoken) && res.reject_ptoken.length > 0) {
                    res.reject_ptoken.forEach((ptoken) => {
                        snupdatedResponse.reject_ptoken.push(ptoken);
                    });
                }
            }
    ...
    async function renderActivities(activities) {
        var activities_html = '';
        var speed_networking_user_count = 0;
        if (activities.length > 0) {
            ...
            for (let activity of activities) {
                ...
                } else if (activity.action == 'speed_networking_count') {
            ...
            if (speed_networking_user_count > 0) {
                const userText = speed_networking_user_count > 1 ? "Users are" : "User is";
                activities_html += `<li class="d-flex align-items-center activity-list" style="gap: 12px;">
                    <div>
                        <p class="fs-12 fw-400 mb-0" style="line-height: 1.149;">
                            <span class="fw-500 text-capitalize">
                                ${speed_networking_user_count}
                            </span> ${userText} in the Speed networking Channel
                        </p>
                    </div>
                </li>`;
            }
    ...
    $(document).on('click', 'li[role="speed"]', function () {
    ...
    $(document).on('click', 'li[role="channel"]', function () {
        ...
        //$("#channelList .channel_btn").first().click();
        $('#channel-chat').removeClass('d-block');
        $('.chat-input-bottom').addClass('d-none');
        $('#channelData-sidebar').removeClass('d-xl-block');
        $('.user-chat').prepend(`<div id="zero-day-div" class="card card-item m-3">
                <div class="card-body" style="text-align: justify;font-size: 20px;">
                    <div class="col-lg-12" style="text-align: center;">
                        <img class="no-network-place" src="${_taoh_cdn_main_prefix + '/images/empty_network.png'}" width="300" alt="no-network">
                    </div>
    ...
    $(document).on('click', 'li[role="direct"]', function () {
        $('#channel-chat-div').removeClass('d-none');
        $('.start-screen').addClass('d-none');
        $("#dmChannelList .channel_btn").first().click();
        taoh_load_network_entries();
    ...
    async function openChannel(channelId, channelType) {
       ...
       CURRENT_CHANNEL_ID = channelId;
        ...
        $('.cw_channel_sub_title1').html("");        
        if(channelInfo?.data?.eventtoken && channelInfo?.data?.exhibitor_id) {
            $('.cw_channel_sub_title1').html(`<a target="_blank" href="${_taoh_site_url_root}/events/exhibitors/${channelInfo.data.exhibitor_id}/${channelInfo.data.eventtoken}">View Detail</a>`);
        }
        if(channelInfo?.data?.eventtoken && channelInfo?.data?.speaker_id) {
            $('.cw_channel_sub_title1').html(`<a target="_blank" href="${_taoh_site_url_root}/events/speaker/${channelInfo.data.eventtoken}/${channelInfo.data.speaker_id}">View Detail</a>`);
        }
