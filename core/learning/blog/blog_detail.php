...
// JSON-LD Article structured data for SEO/AEO
$jsonld_article = array(
    '@context' => 'https://schema.org',
    '@type' => 'Article',
    'headline' => $title,
    'description' => substr(strip_tags($description), 0, 240),
    'image' => $image,
    'datePublished' => isset($date) ? date('Y-m-d', strtotime($date)) : '',
    'author' => array(
        '@type' => 'Person',
        'name' => $author,
    ),
...
if (!empty($categories)) {
    $jsonld_article['articleSection'] = is_array($categories) ? implode(', ', $categories) : $categories;
}
if (!empty($video_link)) {
    $jsonld_article['video'] = array(
        '@type' => 'VideoObject',
        'embedUrl' => $video_link,
    );
}
$additive .= '<script type="application/ld+json">' . json_encode($jsonld_article, JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT) . '</script>';
