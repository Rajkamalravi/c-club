...
if (!$is_my_profile_view) {
    taoh_set_error_message('You can only view your own followers list.');
    taoh_redirect( TAOH_SITE_URL_ROOT);
}
...
?>
    ...
    <div class="directory">
        ...
        <div class="col-md-11 m-auto">
            <div class="d-flex align-items-center follow-heading-wrapper justify-content-between">
                <h2 class="follow-heading text-left mt-3 mb-3"><i class="fa fa-users mr-1"></i>Followers</h2>
                <h6 id="total_list_count_vw" style="display: none;">Total: <span id="total_list_count">0</span></h6>
            </div>

            <div class="row justify-content-center">
                <div class="col-md-12 mt-2" id="followers_profiles_blk">
                    <div class="col-md-12 no_result" id="followers_no_result" style="display: none;"></div>
                    <div class="aw aw-logo aw-loader" id="followers_profiles"></div>
    ...
    <script type="text/javascript">
        ...
        let followersProfiles_isProcessing = false;

        let followers_profiles_loader = $('.aw');
        let followers_profiles = $('#followers_profiles');
        const followersProfilesContainer = document.getElementById('followers_profiles');
        ...
        $(document).ready(function () {
            ...
            $('#searchFilter').on('submit', function (e) {
                ...
                // if (formData.get('query').trim() !== '') {
                    followers_profiles.empty();
                    followers_profiles_loader.awloader('show');
                    currentPage = 1;
                    loadProfiles('search');
                // }
            });
        ...
        function loadProfiles(callFromEvent = 'init') {
            if (followersProfiles_isProcessing) {
                setTimeout(() => {
            ...
            loading = true;
            followersProfiles_isProcessing = true;
            ...
            $.ajax({
                ...
                error: function (xhr, status, error) {
                    ...
                    followers_profiles_loader.awloader('hide');
                    loading = false;
                }
            });
        }
        ...
        async function appendProfiles(response, callFromEvent = 'init') {
            ...
            if (response.success && response.output?.length > 0) {
                $('#followers_no_result').hide();
                followers_profiles.show();
                ...
                for (const userInfo of response.output) {
                    ...
                    const userAvatarSrc = buildAvatarImageOptimistic(userInfo.avatar_image, fallbackSrc, (updatedSrc) => {
                        const avatarImgElem = followers_profiles.find(`.profile-pic[data-ptoken="${userInfo.ptoken}"]`);
                    ...
                    followers_profiles.append(profileCardHtml);
                }
                ...
                const followers_profiles_elem = document.getElementById('followers_profiles');
                if (followers_profiles_elem) {
                    const top = followers_profiles_elem.getBoundingClientRect().top + window.pageYOffset - 300;
                    window.scrollTo({ top, behavior: 'auto' });
                }

                /*if (response.total > 0) {
                    let displayTotalCount = $('#followers_profiles li.directory-v2-list').length;
                ...
                endOfContent = true;
                if (!response.success || (callFromEvent !== 'scroll' && followers_profiles.children('.profile').length === 0)) {
                    followers_profiles.hide();
                    show_empty_result_screen($('#followers_no_result'), callFromEvent);
                }
                // $('#total_list_count_vw').hide();
            }
            followersProfiles_isProcessing = false;
        }
        ...
        function show_empty_result_screen(element, callFromEvent = 'init') {
            ...
            if(callFromEvent === 'search'){
                noResultHtml += `<h3>We couldn't find exactly what you were looking for</h3>
                    ...
                    <a href="${_taoh_site_url_root + '/followers/' + pToken}" class="btn theme-btn mb-4">BROWSE FOLLOWERS</a>`;
            } else {
