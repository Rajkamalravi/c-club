...
<!-- Navbar -->
    ...
    <script>
      ...
      $(document).ready(function () {  
        ...
        <?php if(isset($_GET['temp_api_token']) && $_GET['temp_api_token']){ ?>
        var myCustomData = '2';
        ...
        function handleEvent(e) {
            ...
            if (e.detail == 2) {
                $('#loaderIframe').hide();
                let back_url = getCookieData('<?php echo TAOH_ROOT_PATH_HASH;?>_referral_back_url');
                window.location.href = (back_url != undefined && back_url != '') ? back_url : '<?php echo TAOH_SITE_URL_ROOT; ?>';
            }
        }
        ...
        $(document).on('click', '.live_now_btn', function () {
            ...
            $.ajax({
                url: '<?php echo TAOH_SITE_URL_ROOT.'/livenow/live_now_data'; ?>',
                type: 'GET',
                dataType: 'json',
                data: {},
                beforeSend: function() {
        ...
        $(document).on('click', '.feedback-page', function () {
            var usert = '<?php echo $user_ptoken ?>';
            if (usert) {
                window.location = '<?php echo $taoh_home_url . "/feedback" . $utoken . $cont . "/app/" . $app_name; ?>';
            } else {
                window.location = '<?php echo $taoh_home_url . "/support/" . $app_name . $detail_name; ?>';
            }
        });
            ...
            `);
                ...
                if (email && captcha) {
                 ...
                 `);
                            ...
                            $(formid + ' #iemail').val('');
                            $(formid + ' #captcha').val('');
                            window.open('<?php echo $taoh_home_url . "/login"; ?>', '_self');
                            var data = {
                                ...
                                'network_title': '<?php echo $title ?? 'default'; ?>',
                                'app_name': '',
                                'referral_link': '<?php echo $taoh_home_url . "/login"; ?>',
                            };
                            jQuery.post("<?php echo taoh_site_ajax_url(); ?>", data, function (response) {
                                $("#loader").hide();
                                if (response == 1) {
    ...
    <script>
        function updateTime(timezone) {
            ...
            if (timeSpan) {
                timeSpan.innerHTML = `<i class="fas fa-clock small" date-time="${dateString}"></i><a title="Based on ${timezone} Timezone, visit settings page to change your timezone"
                href="<?php echo TAOH_SITE_URL_ROOT; ?>/settings" style="color: #212121;"> ${timeString} </a>`;
            }
        }

        // Use session timezone or fallback to browser timezone
        const h_userTimezone = "<?php echo isset(taoh_session_get(TAOH_ROOT_PATH_HASH)['USER_INFO']->local_timezone)
            ? taoh_session_get(TAOH_ROOT_PATH_HASH)['USER_INFO']->local_timezone
