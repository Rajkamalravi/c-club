...
$h_directory_flags_to_show = defined('TAOH_DIRECTORY_FLAGS_TO_SHOW') ? TAOH_DIRECTORY_FLAGS_TO_SHOW : [];

$user_data = taoh_user_all_info();
$user_ptoken = ( isset( $user_data->ptoken ) && $user_data->ptoken ) ? $user_data->ptoken:'';
$profile_complete = (isset($user_data->profile_complete) && $user_data->profile_complete) ? $user_data->profile_complete : 0;
$h_my_profile_tag_category = $user_data->tags ?? [];
...
$tokenname = '';
$detail_name = '';$cont = ''; $utoken ='';
if($user_ptoken){
  $utoken = '/pt/'.$user_ptoken;

}
if($conttokenvar != ''){
    $conttokenvar_exp = explode('-', $conttokenvar);
    $conttoken = array_pop($conttokenvar_exp);
...
if(taoh_user_is_logged_in()){
    ...
    $owner_json = taoh_get_user_info(($sess_user_info?->ptoken) ?? $user_ptoken);
  ...
  if ( is_string( $owner_json ) && is_array( json_decode( $owner_json, true ) ) ) {
    ...
    $owner_arrar = $owner_json;
  }
  if ( isset( $owner_arrar['output']['user']['name'] ) && $owner_arrar['output']['user']['name'] ) {
    $data_api = json_encode($owner_arrar['output']['user']);
  } else {
    $data_api = '';
  }
}
...
$ptoken = ( isset( $user_data->ptoken ) && $user_data->ptoken ) ? $user_data->ptoken:'';
...
<header class="page-header header-area bg-white border-bottom border-bottom-gray header1" id="myHeader">
  <nav class="header-nav navbar navbar-expand-lg navbar-light bg-white py-0 px-3" id="mainNavbar">
    <div class="container">
      <div class="">
        <div class="logo-box" style="text-align: left; display: flex; align-items: center;">
          <a href="<?php echo $taoh_home_url . "/../"; ?>" class="logo text-dark" style="text-decoration:none; white-space:nowrap;">
            <img src="<?php echo (defined('TAOH_SITE_FAVICON')) ? TAOH_SITE_FAVICON : TAOH_SITE_FAVICON; ?>" alt="logo" style="vertical-align:middle; margin-right:8px; height: 35px;"><h6 style="display:inline; font-size:25px; margin:0; font-weight:600; vertical-align:middle;"><?php echo TAOH_SITE_NAME_SLUG ?></h6>
          </a>
          <?php
          ...
          ?>
          <?php if(isset($_GET['profile']) && $_GET['profile'] == 'new') { ?>
          <span class="openProfileModal" data-profile_token="<?php echo (taoh_session_get(TAOH_ROOT_PATH_HASH)['USER_INFO'] ?? null)?->ptoken ?? ''; ?>">profile</span>
          <?php } ?>

        </div>
      </div>
      <div class="user-action">
        <div class="off-canvas-menu-toggle icon-element icon-element-xs" data-toggle="tooltip" data-placement="top" title="" data-original-title="Main menu">
      ...
      <?php
        ...
        <div>
          <a target="bank" href="<?php echo TAOH_DASH_PREFIX.'/tao/superadmin/super_admin_form.php'.
            '?data='.$super_token;?>">
            <button class="btn btn-success" id="manage_site" ><i class="la la-lock mr-2"></i> Manage</button>
          </a>
        </div>
      <?php }*/ ?>

      <?php
      $class = stristr($_SERVER['REQUEST_URI'], TAOH_PLUGIN_PATH_NAME . '/apps') ? 'active' : '';
      ...
      if (!isset($_GET['temp_api_token']) && (taoh_parse_url(0) != 'login-success') && (taoh_parse_url(0) != 'createacc') && (taoh_parse_url(0) != 'createsettings')) { ?>
        ...
        ?>
          <li  class="nav-item <?php echo  (    
            ...
            )) && taoh_parse_url(1) != "live") ? "active":"") ?>">
            <a class="nav-link d-flex flex-column text-center" aria-current="page" href="<?php echo $taoh_home_url; ?>"><svg xmlns="http://www.w3.org/2000/svg" width="27.712" height="27" viewBox="0 0 27.712 27.555">
        ...
        <?php } ?>
          ...
          <li class="nav-item <?php echo (( stristr( $_SERVER[ 'REQUEST_URI' ], TAOH_PLUGIN_PATH_NAME.'/events' ) ) ? "active":"") ?>">
            <a class="nav-link d-flex flex-column text-center" onclick="sessionStorage.removeItem('events_search_state')" aria-current="page" href="<?php echo $taoh_home_url."/events"; ?>"><svg xmlns="http://www.w3.org/2000/svg" width="25.27" height="27" viewBox="0 0 25.251 26.169">
          ...
          <!--<li class="nav-item <?php //echo (( stristr( $_SERVER[ 'REQUEST_URI' ], TAOH_PLUGIN_PATH_NAME.'/jobs' ) ) ? "active":"") ?>">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" width="31.051" height="27.691" viewBox="0 0 31.051 27.691">
                    ...
                    aria-labelledby="jobMenuDropdown">
                            ...
                            <div class="dropdown-item-list">
                                    ...
                                    <?php /*if(!taoh_user_is_logged_in()) { ?>
                                      <a class="dropdown-item" href="<?php echo TAOH_SITE_URL_ROOT."/jobs/scout"; ?>">Scout</a>
                                    <?php }*/ ?>
          ...
          <li class="nav-item <?php echo (( stristr( $_SERVER[ 'REQUEST_URI' ], TAOH_PLUGIN_PATH_NAME.'/asks' ) ) ? "active":"") ?>">
            <a class="nav-link d-flex flex-column text-center" aria-current="page" href="<?php echo $taoh_home_url."/asks"; ?>"><svg xmlns="http://www.w3.org/2000/svg" width="33.085" height="27" viewBox="0 0 32.085 27.854">
          ...
          ?>
          <li class="nav-item <?php echo (( stristr( $_SERVER[ 'REQUEST_URI' ], TAOH_PLUGIN_PATH_NAME.'/message' ) ) ? "active":"") ?> d-none">
            <a class="nav-link d-flex flex-column text-center" aria-current="page" href="<?php echo $taoh_home_url."/connections/club"; ?>"><svg xmlns="http://www.w3.org/2000/svg" width="35.712" height="27.555" viewBox="0 0 35.712 27.555">
            <defs id="defs2"/>
          ...
          <?php
            if ( taoh_user_is_logged_in() && TAOH_NOTIFICATION_ENABLED == 1 ) {
          ?>
         ...
         <?php } ?>
            ...
            <li class="dropdown stay_open user-dropdown bor-right">
                <a class="nav-link dropdown-toggle dropdown--toggle pl-2 text-center" href="#" id="userMenuDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        <div class="media media-card media--card shadow-none mb-0 rounded-0 align-items-center bg-transparent">
                                <div class="media-body p-0 border-left-0 text-center active-menu">
                                  <?php
                                  echo '<div>'.taoh_get_profile_image().'</div>';
                                  if (strstr(strtolower($_SERVER['REQUEST_URI']), 'event')) {
                                      echo '<span class="small"><span id="time_span" style="font-size: 15px; font-weight: bold;"></span></span>';
                                  }
                                  ?>
                                </div>
                          </div>
                ...
                <div class="dropdown-menu dropdown--menu dropdown-menu-right mt-3 keep-open" id="userMenuDropdownarea" aria-labelledby="userMenuDropdown">
                        <h6 class="dropdown-header text-wrap">Hi, <?php echo taoh_user_full_name(); ?></h6>
                        <div class="dropdown-divider border-top-gray mb-0"></div>
                        <div class="dropdown-item-list">
                            <?php
                            if ($profile_complete) {
                                echo '<a class="dropdown-item" href="' . $taoh_home_url . '/profile/' . $ptoken . '"><i class="la la-user-tie mr-2"></i>Profile</a>';
                            }
                            ...
                            <div class="pl-4 d-none" id="myNetworkMenu">
                                <?php
                                if (!empty($h_my_profile_tag_category) && !empty($h_directory_flags_to_show)) {
                                    $h_flags_to_show = [];
                                    foreach ($h_my_profile_tag_category as $tag) {
                                        $kebab_selected_flag = str_replace(' ', '-', strtolower($tag));
                                        if (array_key_exists($kebab_selected_flag, $h_directory_flags_to_show)) {
                                            $h_flags_to_show = array_merge($h_flags_to_show, $h_directory_flags_to_show[$kebab_selected_flag]);
                                        }
                                    }

                                    if (!empty($h_flags_to_show)) {
                                        $h_flags_to_show = array_values(array_unique($h_flags_to_show));

                                        foreach ($h_flags_to_show as $flag) {
                                            $kebab_flag = str_replace(' ', '-', strtolower($flag));
                                            echo '<a class="dropdown-item" href="' . $taoh_home_url . '/directory/profile/flag/' . $kebab_flag . '">' . htmlspecialchars($flag) . '</a>';
                                        }
                                    }
                                }

                                ?>
                                <a class="dropdown-item" href="<?= $taoh_home_url ?>/directory/club"><?= defined('TAOH_SITE_NAME_SLUG') ? TAOH_SITE_NAME_SLUG . ' Directory' : 'Directory' ?></a>
                            </div>
                            ...
                            if (taoh_user_is_logged_in() && isset(taoh_session_get(TAOH_ROOT_PATH_HASH)['USER_INFO']->is_super_admin) &&
                                ...
                                <a class="dropdown-item" target="_blank"
                                   href="<?php echo TAOH_DASH_PREFIX . '/tao/superadmin/super_admin_form.php' . '?data=' . $super_token; ?>">
                                    <i class="la la-lock mr-2"></i>Manage Site Settings</a>
                            <?php
                            }

                            //Messages & Referral - hidden until fixed
                            if (TAOH_MESSAGE_ENABLE) {
                                echo '<a class="dropdown-item" href="' . $taoh_home_url . '/dm/networking"><i class="la la-sms mr-2"></i>Messages</a>';
                            }
                            echo '<a class="dropdown-item" href="' . $taoh_home_url . '/referral"><i class="la la-user-plus mr-2"></i>Referral</a>';
          ...
          <?php
            } else if (  taoh_user_is_logged_in()  && defined(  'TAOH_PAGE_AVATAR') && defined(  'TAOH_PAGE_AVATAR') && defined( 'TAOH_PAGE_CHAT_NAME' ) && defined( 'TAOH_PAGE_PTOKEN' ) ){
              ?>
                ...
                <div class="dropdown-menu dropdown--menu dropdown-menu-right mt-3 keep-open" aria-labelledby="userMenuDropdown">
                    <div class="dropdown-item-list">
                         ...
                         <i class="la la-power-off mr-2"></i>Log In</a>-->
                          ...
                          <a onclick="localStorage.removeItem('isCodeSent'); openLoginPopup(); return false;"
                          class="dropdown-item" href="<?php echo  $taoh_home_url."/login.php"; ?>">
                          <i class="la la-power-off mr-2"></i>Log In</a>
                    </div>
                </div>
            </li>
            ...
            <?php
              ...
              if(TAOH_LEARNING_ENABLE) { ?>
                <li class="nav-item dropdown learning-dropdown">
                  <a style="cursor: pointer;" class="nav-link dropdown-toggle d-flex flex-column text-center" id="learningMenuDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" data-supported-dps="24x24" fill="currentColor" class="mercado-match" width="24" height="24" focusable="false">
                    <path d="M3 3h4v4H3zm7 4h4V3h-4zm7-4v4h4V3zM3 14h4v-4H3zm7 0h4v-4h-4zm7 0h4v-4h-4zM3 21h4v-4H3zm7 0h4v-4h-4zm7 0h4v-4h-4z"></path>
                    </svg><span class="small">Learning</span></a>
                  <div class="dropdown-menu dropdown--menu dropdown-menu-right mt-3" aria-labelledby="learningMenuDropdown">
                    <div class="dropdown-item-list">
                      <?php if ( TAOH_READS_ENABLE ) { ?>
                      <a class="dropdown-item" href="<?php echo $taoh_home_url."/learning/"; ?>"><i class="la la-book mr-2"></i>Blog</a>
                      <?php } ?>
                      <a class="dropdown-item" href="<?php echo $taoh_home_url."/learning/flashcard/"; ?>"><i class="la la-clone mr-2"></i>Flashcard</a>
                      <a class="dropdown-item" href="<?php echo $taoh_home_url."/learning/askobviousbaba"; ?>"><i class="la la-user-circle mr-2"></i>Obvious Baba</a>
                      <a class="dropdown-item" href="<?php echo $taoh_home_url."/learning/jusask"; ?>"><i class="la la-comments mr-2"></i>Sidekick</a>
                    </div>
                  </div>
                </li>
                <?php
              ...
              if ( taoh_user_is_logged_in() && defined( 'TAOH_SITE_DONATE_ENABLE' ) && TAOH_SITE_DONATE_ENABLE ) { ?>
                <li style="color:#2d86eb;font-weight:bold;" class="nav-item <?php echo (( stristr( $_SERVER[ 'REQUEST_URI' ], TAOH_PLUGIN_PATH_NAME.'/network' ) ) ? "active":"") ?>">
                <a class="nav-link d-flex flex-column text-center" aria-current="page" href="<?php echo TAOH_SITE_URL_ROOT."/donate"; ?>">
                <img width="26.107" height="24" src="<?php echo TAOH_SITE_URL_ROOT."/assets/images/donate_menu.png"; ?>">
                <span style="color:#2d86eb;font-weight:bold;" class="small">DONATE</span></a>
                </li>
                <?php
              }

              if (TAOH_ENABLE_SEPARATE_EMPLOYER && ! taoh_user_is_logged_in()){ ?>
              ...
              if ( TAOH_ENABLE_SEPARATE_EMPLOYER && taoh_user_is_logged_in() && (!isset( $_SESSION[ TAOH_ROOT_PATH_HASH ][ 'USER_INFO' ]->type ) || $_SESSION[ TAOH_ROOT_PATH_HASH ][ 'USER_INFO' ]->type == 'employer') ){ ?>
                  <li style="color:#2d86eb;font-weight:bold;" class="nav-item <?php echo (( stristr( $_SERVER[ 'REQUEST_URI' ], TAOH_PLUGIN_PATH_NAME.'/employers' ) ) ? "active":"") ?>">
                    <a class="nav-link d-flex flex-column text-center" aria-current="page" href="<?php echo TAOH_SITE_URL_ROOT.'/jobs/dash'; ?>">
                    <img width="30" height="24px" src="<?php echo TAOH_SITE_URL_ROOT."/assets/images/employer_menu_9d0f54.png"; ?>">
                    <span class="small" style="color: #9d0f54;font-weight:bold;">Employers</span></a>
                  </li>
                  <?php
              }
          if(taoh_user_is_logged_in() && !stristr($_SERVER['REQUEST_URI'], TAOH_PLUGIN_PATH_NAME.'/events') && !stristr($_SERVER['REQUEST_URI'], TAOH_PLUGIN_PATH_NAME.'/message/dm')) { ?>
          <li style="color:#2d86eb;font-weight:bold;" class="nav-item d-none d-lg-block live_now_block">
            <a class="nav-link d-flex flex-column text-center live_now_btn" aria-current="page" style="cursor: pointer;"  >
              <div class="circle-highlight" style="width: 24px; height: 24px;">
                  <div class="main-circle"></div>
                  <div class="center-circle" style="width: 12.5px; height: 12.5px;"></div>
              </div>
              <span class="small" style="color: #9d0f54;font-weight:bold;">Live</span></a>
            </a>
          </li>
          <?php } ?>
  ...
  <div class="off-canvas-menu custom-scrollbar-styled">
        <div class="off-canvas-menu-close icon-element icon-element-sm shadow-sm" data-toggle="tooltip" data-placement="left" title="" data-original-title="Close menu">
        ...
        <ul class="generic-list-item off-canvas-menu-list pt-90px">
          ...
          <!--<li class="nav-item">
            <a class="nav-link text-center" aria-current="page" href="<?php echo $taoh_home_url; ?>"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" data-supported-dps="24x24" fill="currentColor" class="mercado-match" width="26" height="26" focusable="false">
          ...
          <li class="nav-item <?php echo (((
              ...
              stristr($_SERVER['REQUEST_URI'], TAOH_PLUGIN_PATH_NAME.'/networking') ||
              stristr($_SERVER['REQUEST_URI'], TAOH_PLUGIN_PATH_NAME.'/room')
              ) && !stristr($_SERVER['REQUEST_URI'], TAOH_PLUGIN_PATH_NAME.'/club/club/live/')
            ) ? "active" : ""); ?>">
              <a class="nav-link text-center" aria-current="page" href="<?php echo $taoh_home_url; ?>"><svg xmlns="http://www.w3.org/2000/svg" width="27.712" height="27.555" viewBox="0 0 27.712 27.555">
                <g id="Layer_2" data-name="Layer 2" transform="translate(-0.032 0.003)">
                  <g id="Layer_1" data-name="Layer 1" transform="translate(0.457 0.422)">
                    <path id="Path_351" data-name="Path 351" d="M13.3.459a2.6,2.6,0,0,0-1.063.631,2.631,2.631,0,0,0-.376.6,1.5,1.5,0,0,0-.173.872,1.5,1.5,0,0,0,.173.876,2.067,2.067,0,0,0,1,1.041,2.322,2.322,0,0,0,.451.188c.06,0,.071.162.06.9v.906l-.395.038a7.381,7.381,0,0,0-3.668,1.5l-.342.267-2.029-2-2-2,.105-.233A1.92,1.92,0,0,0,5.02,2.68a1.879,1.879,0,0,0-.834-.834,1.958,1.958,0,0,0-1.458,0,1.924,1.924,0,0,0-.834.849,1.958,1.958,0,0,0,0,1.458,1.924,1.924,0,0,0,.834.834,1.92,1.92,0,0,0,1.364.026L4.325,4.9l2,2.007,2,2-.271.346a7.414,7.414,0,0,0-1.5,3.668l-.038.376-.466.019H5.58l-.03-.188A2.8,2.8,0,0,0,4.829,11.9a2.54,2.54,0,0,0-3.641.056,2.544,2.544,0,0,0,.068,3.634,2.533,2.533,0,0,0,3.574,0,2.818,2.818,0,0,0,.718-1.2l.03-.188h.936l.038.376a7.546,7.546,0,0,0,1.417,3.559,8.538,8.538,0,0,0,2.21,1.939,7.659,7.659,0,0,0,2.777.917l.406.038v.9c0,.752,0,.906-.06.906a2.322,2.322,0,0,0-.451.188,2.067,2.067,0,0,0-1,1.041,1.5,1.5,0,0,0-.173.879,1.5,1.5,0,0,0,.2.921,2.074,2.074,0,0,0,1.063,1.079,1.5,1.5,0,0,0,.872.173,1.5,1.5,0,0,0,.876-.173,2.089,2.089,0,0,0,1.056-1.03,1.5,1.5,0,0,0,.2-.921,1.5,1.5,0,0,0-.173-.879,2.067,2.067,0,0,0-1-1.041,2.322,2.322,0,0,0-.451-.188c-.06,0-.071-.162-.06-.9v-.906l.376-.038a7.482,7.482,0,0,0,3.63-1.481l.376-.3.962.924.924.921-.105.233a1.706,1.706,0,0,0,.861,2.2,1.958,1.958,0,0,0,1.458,0,1.729,1.729,0,0,0,.853-2.292,1.905,1.905,0,0,0-.834-.834,1.943,1.943,0,0,0-1.42-.023l-.2.109-.924-.928-.921-.928.271-.338a7.478,7.478,0,0,0,1.5-3.671l.038-.376h.932l.034.188a2.818,2.818,0,0,0,.722,1.229,2.57,2.57,0,1,0,0-3.687,2.8,2.8,0,0,0-.7,1.18l-.034.188h-.466l-.466-.019-.038-.376a7.414,7.414,0,0,0-1.5-3.668L19.3,8.948l.9-.958.921-.928.233.109a1.5,1.5,0,0,0,.661.109,1.2,1.2,0,0,0,.782-.2,1.717,1.717,0,0,0-.068-3.063A1.357,1.357,0,0,0,22,3.886a1.357,1.357,0,0,0-.729.135,1.879,1.879,0,0,0-.834.834,1.95,1.95,0,0,0-.023,1.39l.109.207-.928.921-.928.921-.376-.3a7.448,7.448,0,0,0-3.63-1.481l-.376-.038V5.57c0-.752,0-.9.06-.9A2.322,2.322,0,0,0,14.8,4.48a2.067,2.067,0,0,0,1-1.041,1.5,1.5,0,0,0,.173-.879,1.5,1.5,0,0,0-.2-.921A2.089,2.089,0,0,0,13.927.425,3.232,3.232,0,0,0,13.3.459Zm.906.872A1.432,1.432,0,0,1,15.1,2.56a1.3,1.3,0,1,1-2.589,0,1.172,1.172,0,0,1,.406-.887A1.2,1.2,0,0,1,14.208,1.331ZM3.8,2.639a.849.849,0,1,1-1.184.6.868.868,0,0,1,1.184-.6ZM22.341,4.792A.838.838,0,0,1,22.6,6.175a.8.8,0,0,1-1.191,0,.853.853,0,0,1,.936-1.383ZM14.825,7.385a6.475,6.475,0,0,1,5.122,8.4,5.4,5.4,0,0,1-.725,1.481c-.034,0-.086-.086-.124-.21a7.114,7.114,0,0,0-.706-1.447,6.764,6.764,0,0,0-1.747-1.65c-.259-.158-.5-.293-.534-.3s0-.105.109-.237A3.006,3.006,0,0,0,15.133,8.9a2.912,2.912,0,0,0-2.631,0,3.006,3.006,0,0,0-1.109,4.513.778.778,0,0,1,.147.225l-.346.188a5.8,5.8,0,0,0-2.631,3.149c-.053.165-.124.3-.15.289a5.241,5.241,0,0,1-.725-1.481A6.475,6.475,0,0,1,14.817,7.4Zm-.132,2.274a2.074,2.074,0,0,1,1.045,1.026,1.526,1.526,0,0,1,.2.947,1.661,1.661,0,0,1-.116.8,2.484,2.484,0,0,1-.838.909.462.462,0,0,0-.312.451c0,.218.147.376.47.447a6.445,6.445,0,0,1,1.379.68,4.96,4.96,0,0,1,1.9,2.92l.083.376-.338.312a6.708,6.708,0,0,1-3.074,1.586,8.9,8.9,0,0,1-2.567,0,6.655,6.655,0,0,1-3.074-1.586l-.342-.312.079-.376a4.78,4.78,0,0,1,3.236-3.585c.376-.124.522-.252.522-.474a.462.462,0,0,0-.312-.436,2.484,2.484,0,0,1-.838-.909,1.646,1.646,0,0,1-.116-.793,1.533,1.533,0,0,1,.173-.9,2.082,2.082,0,0,1,.924-1,1.879,1.879,0,0,1,1.15-.259A1.8,1.8,0,0,1,14.693,9.658ZM3.754,12.217a1.879,1.879,0,0,1,.834.823,1.334,1.334,0,0,1,.135.752,1.266,1.266,0,0,1-.184.8,1.556,1.556,0,0,1-.782.752,1.251,1.251,0,0,1-.733.143,1.312,1.312,0,0,1-.725-.132,1.732,1.732,0,0,1,.188-3.2,1.879,1.879,0,0,1,1.266.064Zm21.563,0a1.905,1.905,0,0,1,.838.823,1.375,1.375,0,0,1,.132.752,1.289,1.289,0,0,1-.18.8,1.586,1.586,0,0,1-.782.752,1.266,1.266,0,0,1-.733.143,1.319,1.319,0,0,1-.729-.132,1.732,1.732,0,0,1,.188-3.2A1.879,1.879,0,0,1,25.317,12.217ZM22.341,21.18a.838.838,0,0,1,.256,1.383.8.8,0,0,1-1.191,0,.853.853,0,0,1,.936-1.383Zm-8.132,2.578a1.428,1.428,0,0,1,.894,1.229,1.3,1.3,0,0,1-2.589,0,1.349,1.349,0,0,1,1.695-1.229Z" transform="translate(-0.457 -0.422)" stroke="currentColor" stroke-width="0.85" fill="currentColor" class="mercado-match" focusable="false" stroke-miterlimit="10"/>
                    <path id="Path_352" data-name="Path 352" d="M62.936,5.541c-.473.477-.556.586-.556.729a.455.455,0,0,0,.443.443c.143,0,.252-.083.725-.556s.567-.582.567-.729A.462.462,0,0,0,63.661,5C63.519,5,63.41,5.068,62.936,5.541Z" transform="translate(-39.11 -3.28)" stroke="currentColor" stroke-width="0.85" fill="currentColor" class="mercado-match" focusable="false" stroke-miterlimit="10"/>
          ...
          <li class="nav-item <?php echo (( stristr( $_SERVER[ 'REQUEST_URI' ], TAOH_PLUGIN_PATH_NAME.'/jobs' ) ) ? "active":"") ?>">
            <a class="nav-link text-center" aria-current="page" href="<?php echo TAOH_SITE_URL_ROOT."/jobs"; ?>">
            <svg xmlns="http://www.w3.org/2000/svg" width="31.051" height="27.691" viewBox="0 0 31.051 27.691">
              <g id="Layer_2" data-name="Layer 2" transform="translate(0 -0.005)">
                <g id="Layer_1" data-name="Layer 1" transform="translate(0.426 0.43)">
                  <path id="Path_355" data-name="Path 355" d="M30.626,15.735c-.078.39-.14.779-.242,1.169a7.057,7.057,0,0,1-1.847,3.265.756.756,0,0,0-.222.581,8.223,8.223,0,0,1-.086,1.582,2.985,2.985,0,0,1-2.993,2.4c-2.291.027-4.578,0-6.87,0-.351,0-.542-.168-.538-.448s.187-.433.549-.433H25.1a2.17,2.17,0,0,0,2.241-1.6,8.806,8.806,0,0,0,.136-1.13,7.512,7.512,0,0,1-8.358,0c-.269.273-.553.557-.826.849a.351.351,0,0,0-.055.238,1.033,1.033,0,0,1-.316.826C16.75,24.2,15.55,25.418,14.35,26.6a1.828,1.828,0,0,1-2.583-2.587c1.169-1.2,2.377-2.381,3.565-3.569a1.017,1.017,0,0,1,.826-.308.421.421,0,0,0,.277-.1c.277-.253.534-.522.818-.8a7.427,7.427,0,0,1-1.294-4.165h-7.4v1.559a1.66,1.66,0,0,1-3.308.242c-.039-.436-.019-.881-.023-1.325v-.5a5.334,5.334,0,0,1-3.9-2.256c0,.132,0,.214,0,.3v8.467a2.178,2.178,0,0,0,2.291,2.3h6.1c.421,0,.635.156.627.448s-.218.436-.643.436H3.673A3.077,3.077,0,0,1,.427,21.471V8.11A3.047,3.047,0,0,1,2.836,5.032a4.742,4.742,0,0,1,.908-.094H8.867v-.39a11.928,11.928,0,0,1,.121-1.874A2.829,2.829,0,0,1,11.716.457Q14.327.406,16.953.449a2.95,2.95,0,0,1,2.922,3.035V4.95h5.249a3.045,3.045,0,0,1,3.117,2.412,11.689,11.689,0,0,1,.086,2.026.779.779,0,0,0,.23.569,7.158,7.158,0,0,1,2.015,4.255,2.124,2.124,0,0,0,.055.226ZM27.466,9a11.078,11.078,0,0,0-.113-1.5,2.174,2.174,0,0,0-2.3-1.664H3.436A2.186,2.186,0,0,0,1.312,8.075V9.785a4.236,4.236,0,0,0,1.711,3.507,4.1,4.1,0,0,0,2.213.861,1.917,1.917,0,0,1,.39-1.407,1.656,1.656,0,0,1,1.29-.6A1.621,1.621,0,0,1,8,12.532a1.824,1.824,0,0,1,.581,1.6H16A7.4,7.4,0,0,1,27.466,9ZM16.848,15.069a6.449,6.449,0,1,0,6.461-6.457A6.449,6.449,0,0,0,16.848,15.069ZM18.963,4.938c0-.623.047-1.216,0-1.8a1.991,1.991,0,0,0-1.948-1.808c-1.75-.027-3.507-.023-5.245,0A2,2,0,0,0,9.9,2.717,8.431,8.431,0,0,0,9.775,4.93h.947V3.415A1.111,1.111,0,0,1,11.8,2.273q2.591-.023,5.186,0a1.122,1.122,0,0,1,1.079,1.169V4.677a1.111,1.111,0,0,0,.019.253Zm-7.38,0h5.58V3.617c0-.39-.058-.464-.456-.464H11.95c-.191,0-.343.066-.347.277C11.56,3.925,11.583,4.424,11.583,4.938Zm3.3,17.211c-.865.865-1.7,1.687-2.525,2.529a.92.92,0,0,0,0,1.3.947.947,0,0,0,1.305.039c.156-.136.3-.288.44-.433l2.085-2.081a1.629,1.629,0,0,0-.1-.132Zm-8.74-6.893h1.52V13.7A.748.748,0,0,0,6.915,13a.725.725,0,0,0-.779.616C6.1,14.165,6.144,14.707,6.144,15.256Zm.019.923a.982.982,0,0,0,.3,1.1.779.779,0,0,0,.861,0c.409-.277.39-.686.339-1.114Zm11.191,6.145a.355.355,0,0,0-.043-.082l-.779-.779c-.5-.5-.5-.5-.97,0h0l1.325,1.321ZM18.359,20.6l-.526-.623-.779.7.6.627Z" transform="translate(-0.426 -0.43)" stroke="currentColor" stroke-width="0.85" fill="currentColor" class="mercado-match" focusable="false" stroke-miterlimit="10"/>
          ...
          <!-- <li class="nav-item dropdown <?php echo (( stristr( $_SERVER[ 'REQUEST_URI' ], TAOH_PLUGIN_PATH_NAME.'/jobs' ) ) ? "active":"") ?>">
            <a class="nav-link text-center" aria-current="page" href="<?php echo $taoh_home_url."/jobs"; ?>" id="jobMenuDropdown" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
            <svg xmlns="http://www.w3.org/2000/svg" width="31.051" height="27.691" viewBox="0 0 31.051 27.691">
              <g id="Layer_2" data-name="Layer 2" transform="translate(0 -0.005)">
                <g id="Layer_1" data-name="Layer 1" transform="translate(0.426 0.43)">
                  <path id="Path_355" data-name="Path 355" d="M30.626,15.735c-.078.39-.14.779-.242,1.169a7.057,7.057,0,0,1-1.847,3.265.756.756,0,0,0-.222.581,8.223,8.223,0,0,1-.086,1.582,2.985,2.985,0,0,1-2.993,2.4c-2.291.027-4.578,0-6.87,0-.351,0-.542-.168-.538-.448s.187-.433.549-.433H25.1a2.17,2.17,0,0,0,2.241-1.6,8.806,8.806,0,0,0,.136-1.13,7.512,7.512,0,0,1-8.358,0c-.269.273-.553.557-.826.849a.351.351,0,0,0-.055.238,1.033,1.033,0,0,1-.316.826C16.75,24.2,15.55,25.418,14.35,26.6a1.828,1.828,0,0,1-2.583-2.587c1.169-1.2,2.377-2.381,3.565-3.569a1.017,1.017,0,0,1,.826-.308.421.421,0,0,0,.277-.1c.277-.253.534-.522.818-.8a7.427,7.427,0,0,1-1.294-4.165h-7.4v1.559a1.66,1.66,0,0,1-3.308.242c-.039-.436-.019-.881-.023-1.325v-.5a5.334,5.334,0,0,1-3.9-2.256c0,.132,0,.214,0,.3v8.467a2.178,2.178,0,0,0,2.291,2.3h6.1c.421,0,.635.156.627.448s-.218.436-.643.436H3.673A3.077,3.077,0,0,1,.427,21.471V8.11A3.047,3.047,0,0,1,2.836,5.032a4.742,4.742,0,0,1,.908-.094H8.867v-.39a11.928,11.928,0,0,1,.121-1.874A2.829,2.829,0,0,1,11.716.457Q14.327.406,16.953.449a2.95,2.95,0,0,1,2.922,3.035V4.95h5.249a3.045,3.045,0,0,1,3.117,2.412,11.689,11.689,0,0,1,.086,2.026.779.779,0,0,0,.23.569,7.158,7.158,0,0,1,2.015,4.255,2.124,2.124,0,0,0,.055.226ZM27.466,9a11.078,11.078,0,0,0-.113-1.5,2.174,2.174,0,0,0-2.3-1.664H3.436A2.186,2.186,0,0,0,1.312,8.075V9.785a4.236,4.236,0,0,0,1.711,3.507,4.1,4.1,0,0,0,2.213.861,1.917,1.917,0,0,1,.39-1.407,1.656,1.656,0,0,1,1.29-.6A1.621,1.621,0,0,1,8,12.532a1.824,1.824,0,0,1,.581,1.6H16A7.4,7.4,0,0,1,27.466,9ZM16.848,15.069a6.449,6.449,0,1,0,6.461-6.457A6.449,6.449,0,0,0,16.848,15.069ZM18.963,4.938c0-.623.047-1.216,0-1.8a1.991,1.991,0,0,0-1.948-1.808c-1.75-.027-3.507-.023-5.245,0A2,2,0,0,0,9.9,2.717,8.431,8.431,0,0,0,9.775,4.93h.947V3.415A1.111,1.111,0,0,1,11.8,2.273q2.591-.023,5.186,0a1.122,1.122,0,0,1,1.079,1.169V4.677a1.111,1.111,0,0,0,.019.253Zm-7.38,0h5.58V3.617c0-.39-.058-.464-.456-.464H11.95c-.191,0-.343.066-.347.277C11.56,3.925,11.583,4.424,11.583,4.938Zm3.3,17.211c-.865.865-1.7,1.687-2.525,2.529a.92.92,0,0,0,0,1.3.947.947,0,0,0,1.305.039c.156-.136.3-.288.44-.433l2.085-2.081a1.629,1.629,0,0,0-.1-.132Zm-8.74-6.893h1.52V13.7A.748.748,0,0,0,6.915,13a.725.725,0,0,0-.779.616C6.1,14.165,6.144,14.707,6.144,15.256Zm.019.923a.982.982,0,0,0,.3,1.1.779.779,0,0,0,.861,0c.409-.277.39-.686.339-1.114Zm11.191,6.145a.355.355,0,0,0-.043-.082l-.779-.779c-.5-.5-.5-.5-.97,0h0l1.325,1.321ZM18.359,20.6l-.526-.623-.779.7.6.627Z" transform="translate(-0.426 -0.43)" stroke="currentColor" stroke-width="0.85" fill="currentColor" class="mercado-match" focusable="false" stroke-miterlimit="10"/>
          ...
          <a class="nav-link text-center" aria-current="page" href="<?php echo $taoh_home_url."/connections/club" ?>"><svg xmlns="http://www.w3.org/2000/svg" width="35.712" height="27.555" viewBox="0 0 35.712 27.555">
            <defs id="defs2"/>
              <g id="layer1" transform="translate(-396,-292)">
         ...
         <li class="nav-item <?php echo (( stristr( $_SERVER[ 'REQUEST_URI' ], TAOH_PLUGIN_PATH_NAME.'/support' ) ) ? "active":"") ?>">
          ...
          <?php if(TAOH_LEARNING_ENABLE) { ?>
            <li class="nav-item dropdown learning-dropdown">
              <a class="nav-link cursor-pointer text-center dropdown-toggle" id="learningMenuDropdown2" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" data-supported-dps="24x24" fill="currentColor" class="mercado-match" width="24" height="24" focusable="false">
                <path d="M3 3h4v4H3zm7 4h4V3h-4zm7-4v4h4V3zM3 14h4v-4H3zm7 0h4v-4h-4zm7 0h4v-4h-4zM3 21h4v-4H3zm7 0h4v-4h-4zm7 0h4v-4h-4z"></path>
                </svg><span class="small">Learning</span></a>
              <div class="dropdown-menu dropdown--menu dropdown-menu-right mt-3" aria-labelledby="learningMenuDropdown2">
                <div class="dropdown-item-list">
                  <?php if ( TAOH_READS_ENABLE ) { ?>
                  <a class="dropdown-item" href="<?php echo $taoh_home_url."/learning/"; ?>"><i class="la la-book mr-2"></i>Blog</a>
                  <?php } ?>
          ...
          <li style="color:#2d86eb;font-weight:bold;" class="nav-item <?php echo (( stristr( $_SERVER[ 'REQUEST_URI' ], TAOH_PLUGIN_PATH_NAME.'/employers' ) ) ? "active":"") ?>">
            ...
            <img width="30" height="" src="<?php echo TAOH_SITE_URL_ROOT."/assets/images/employer_menu_light.png"; ?>">
            <span class="small">Employers</span></a>
          </li>
          <?php } ?>
          <?php if ( taoh_user_is_logged_in() && TAOH_SITE_DONATE_ENABLE && taoh_parse_url(2) == 'rsvp') { ?>
            <li style="color:#2d86eb;font-weight:bold;" class="nav-item <?php echo (( stristr( $_SERVER[ 'REQUEST_URI' ], TAOH_PLUGIN_PATH_NAME.'/network' ) ) ? "active":"") ?>">
            <a class="nav-link cursor-pointer text-center" aria-current="page" href="<?php echo TAOH_SITE_URL_ROOT."/donate"; ?>">
            <img width="26.107" height="26.107" src="<?php echo TAOH_SITE_URL_ROOT."/assets/images/donate_menu.png"; ?>">
            <span style="color:#2d86eb;font-weight:bold;" class="small">DONATE</span></a>
          ...
          <?php if(taoh_user_is_logged_in() && !stristr($_SERVER['REQUEST_URI'], TAOH_PLUGIN_PATH_NAME.'/events')) { ?>
          <li style="color:#2d86eb;font-weight:bold;" class="nav-item">
            <a class="nav-link live_now_btn" aria-current="page" style="cursor: pointer;"  >
              <div class="circle-highlight" style="width: 24px; height: 24px;">
                  <div class="main-circle"></div>
                  <div class="center-circle" style="width: 12.5px; height: 12.5px;"></div>
              </div>
...
<div class="modal left fade" id="profileModal" tabindex="" role="dialog" aria-labelledby="profileModal" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
      ...
      <div class="modal-body profileModalBody p-0">
        <div id="profileModalContent" data-ptoken="">
...
<!-- Live Now Modal -->
<div class="modal fade" id="liveNowModal" tabindex="-1" role="dialog" aria-labelledby="liveNowModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content modal-header-colored border-0 aw aw-logo aw-loader" id="liveNowModalContent">
            <div class="modal-header" style="font-weight: bold; font-size: 18px;">LIVE</h5>
            ...
            <div class="modal-body p-4">
                <p><b>You're about to join a global Live Now session.</b></p>
                This space connects professionals across our global network, in real-time.<br>
                Explore trending topics like <i>LinkedIn optimization, authentic branding,</i> and more.<br>
                ✅ Review, connect, and grow — because no one should navigate success alone.<br><br>
                <p><b>Proceed to Live Now?</b></p>
            </div>
            <div class="modal-footer border-top-dashed">
                <button type="button" class="btn link-danger mr-5" data-dismiss="modal" data-bs-dismiss="modal"> Not Now</button>
                <a target="_blank" href="#" class="btn btn-primary m-0" id="live_join_now_btn"><i class="fa fa-dot-circle-o mr-2" aria-hidden="true"></i> Join Now</a>
            </div>
        </div>
    </div>
</div>
...
<!-- Navbar -->
    ...
    <link rel="stylesheet" href="<?php echo TAOH_SITE_URL_ROOT; ?>/assets/css/header.css?v=<?php echo TAOH_CSS_JS_VERSION; ?>">

    <script>
    window._hdr_cfg = {
        dataApi: <?php echo json_encode($data_api ?? ''); ?>,
        hasTempApiToken: <?php echo json_encode(!empty($_GET['temp_api_token'])); ?>,
        rootPathHash: <?php echo json_encode(defined('TAOH_ROOT_PATH_HASH') ? TAOH_ROOT_PATH_HASH : ''); ?>,
        siteUrlRoot: <?php echo json_encode(defined('TAOH_SITE_URL_ROOT') ? TAOH_SITE_URL_ROOT : ''); ?>,
        supportPageUrl: <?php echo json_encode(($taoh_home_url ?? '') . "/support/" . ($app_name ?? '') . ($detail_name ?? '')); ?>,
        feedbackUrl: <?php echo json_encode(($taoh_home_url ?? '') . "/feedback" . ($utoken ?? '') . ($cont ?? '') . "/app/" . ($app_name ?? '')); ?>,
        userPtoken: <?php echo json_encode($user_ptoken ?? ''); ?>,
        siteTitle: <?php echo json_encode(defined('TAOH_SITE_TITLE') ? TAOH_SITE_TITLE : ''); ?>,
        ajaxUrl: <?php echo json_encode(taoh_site_ajax_url()); ?>,
        loginUrl: <?php echo json_encode(($taoh_home_url ?? '') . "/login"); ?>,
        networkTitle: <?php echo json_encode($title ?? 'default'); ?>,
        userTimezone: <?php echo json_encode(isset(taoh_session_get(TAOH_ROOT_PATH_HASH)['USER_INFO']->local_timezone) ? taoh_session_get(TAOH_ROOT_PATH_HASH)['USER_INFO']->local_timezone : ''); ?>,
        sessionPtoken: <?php echo json_encode((taoh_session_get(TAOH_ROOT_PATH_HASH)['USER_INFO'] ?? null)?->ptoken ?? ''); ?>
                            };
    </script>
    <script src="<?php echo TAOH_SITE_URL_ROOT; ?>/assets/js/header.js?v=<?php echo TAOH_CSS_JS_VERSION; ?>"></script>
