 ...
 *
 * PORTABLE: Can be drag-dropped to any domain - just update the configuration below
 */

// ========== CONFIGURATION - UPDATE THESE VALUES FOR YOUR SITE ==========

// Site configuration
if (!defined('SITE_NAME')) define('SITE_NAME', 'Unmeta');
if (!defined('SITE_URL')) define('SITE_URL', 'https://unmeta.net/club');
if (!defined('SITE_EMAIL')) define('SITE_EMAIL', 'info@tao.ai');
if (!defined('BASE_URL')) define('BASE_URL', 'https://unmeta.net/club');
...
if (!defined('APP_EMOJI')) define('APP_EMOJI', '⛵');
if (!defined('APP_LOGO_URL')) define('APP_LOGO_URL', 'https://cdn.tao.ai/assets/wertual/images/worker1_sq.png');
if (!defined('SITE_LOGO')) define('SITE_LOGO', APP_LOGO_URL);
if (!defined('POST_LOGIN_URL')) define('POST_LOGIN_URL', SITE_URL.'/sso-login');
if (!defined('SSO_BASE_URL')) define('SSO_BASE_URL', 'https://login.tao.ai');

// SSO Callback Configuration
if (!defined('SSO_CALLBACK_URL')) define('SSO_CALLBACK_URL', SITE_URL . '/login-new.php');
if (!defined('SSO_CALLBACK_FAIL_URL')) define('SSO_CALLBACK_FAIL_URL', SITE_URL . '/login-new.php?error=1');
if (!defined('SSO_VALIDATE_URL')) define('SSO_VALIDATE_URL', SSO_BASE_URL . '/sso.php');

// Cookie Configuration (aligned with SSO server)
// Use '.yourdomain.com' to share cookies across subdomains, or leave empty for current host only
if (!defined('COOKIE_DOMAIN')) define('COOKIE_DOMAIN', '.unmeta.net');
if (!defined('SSO_USER_KEY_COOKIE')) define('SSO_USER_KEY_COOKIE', 'user_key');
if (!defined('SSO_EMAIL_COOKIE')) define('SSO_EMAIL_COOKIE', 'email');
if (!defined('SSO_LOGIN_COOKIE')) define('SSO_LOGIN_COOKIE', 'login_vars');
...
function set_user_cookies($email, $user_key, $domain, $profile_info) {
    ...
    error_log('[login-new.php] Setting cookies - Email: ' . $email . ', Domain: ' . $domain . ', Secure: YES');
    ...
    error_log('[login-new.php] Cookies set - Results: tao_email=' . ($result1?'OK':'FAIL') . ', tao_key=' . ($result2?'OK':'FAIL') . ', logged_in=' . ($result3?'OK':'FAIL') . ', email=' . ($result4?'OK':'FAIL') . ', user_key=' . ($result5?'OK':'FAIL'));
}
...
function post_request($url, $params = []) {
    ...
    curl_close($ch);
    if ($err) {
        error_log('[login-new.php] cURL error: ' . $err);
        return false;
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!validate_ops($ops)) {
        error_log('[login-new.php] ERROR: Invalid ops parameter: ' . $ops);
    ...
    if (!$email || !$user_key) {
        error_log('[login-new.php] ERROR: Missing email or user_key');
        $fail_url = defined('SSO_CALLBACK_FAIL_URL') ? SSO_CALLBACK_FAIL_URL : '/';
    ...
    $validate_params = [
        'email' => $email,
        'user_key' => $user_key,
        'callbackurl' => $site_url . '/login-new.php',
        'ops' => 'validate'
    ];

    error_log('[login-new.php] Validating with: ' . $sso_validate_url);
    $resp = post_request($sso_validate_url, $validate_params);
    error_log('[login-new.php] Validation response: ' . print_r($resp, true));

    // If we have email and user_key from callback, proceed to set cookies
    if ($email && $user_key) {
        error_log('[login-new.php] ========== VALIDATION SUCCESSFUL ==========');
        ...
        if ($fwd_url) {
            ...
            error_log('[login-new.php] Using fwd_url from query parameter: ' . $redirect_url);
        } else if ($fwd_redirect_url) {
            $redirect_url = $fwd_redirect_url;
            error_log('[login-new.php] Using redirect_url from POST: ' . $redirect_url);
        } else {
            $redirect_url = defined('POST_LOGIN_URL') ? POST_LOGIN_URL : (defined('SITE_URL') ? SITE_URL : '/');
            error_log('[login-new.php] Using default redirect URL: ' . $redirect_url);
        }

        header('Location: ' . $redirect_url);
        exit;
    } else {
        error_log('[login-new.php] ERROR: Validation failed');
...
if (is_logged_in() && $fwd_url) {
    error_log('[login-new.php] User logged in with fwd_url - Forwarding to: ' . $fwd_url);
    ...
    error_log('[login-new.php] Forwarding with payload: ' . print_r($payload, true));
...
if ($fwd_url) {
    ...
    error_log('[login-new.php] Not logged in with fwd_url - Will redirect to: ' . $fwd_url . ' after login');
} else {
    $payload = [
        'site_name' => defined('APP_NAME') ? APP_NAME : 'Labs Login',
        'site_logo' => defined('APP_LOGO_URL') ? APP_LOGO_URL : '',
...
error_log('[login-new.php] Redirecting to login.tao.ai with payload: ' . print_r($payload, true));
?>
<!DOCTYPE html>
<html>
<head>
