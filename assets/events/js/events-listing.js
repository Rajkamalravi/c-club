const isLoggedIn = window._evl_cfg.isLoggedIn;
const isValidUser = window._evl_cfg.isValidUser;
const is_rsvp = 0;
const dojoeventrules = window._evl_cfg.dojoEventRules;
...
let currentMod = window._evl_cfg.appSlug;
let geohash = "";
let search = "";
...
let itemsPerPage = 12;
let currentPage = 1;
let like_min = window._evl_cfg.likeMin;
let comment_min = window._evl_cfg.commentMin;
let share_min = window._evl_cfg.shareMin;
let app_slug = 'events';
let arr_cont = [];
let liked_arr = window._evl_cfg.likedArr;
...
var event_type = '';
var user_ptoken = window._evl_cfg.userPtoken;
const rsvped_data = window._evl_cfg.rsvpedData;
const rsvp_find = new Array();
let rsvp_events_list;
let nextRsvpEvent;

let _taoh_static_ajax_token = window._evl_cfg.staticAjaxToken;
...
$(document).ready(function () {
    ...
    setTimeout(function() {
        ...
        if (savedState) {
            ...
            geohash = searchState.geohash || '';
            postDate = searchState.postdate || '';
            from_date = searchState.from_date || '';
            to_date = searchState.to_date || '';

            if (window._evl_cfg.intaoDbEnable) {
    ...
    $('#locationSelect-ts-control').on('click', async function () {
        setTimeout(async () => {
            let locationSelect = $('#locationSelect')[0].tomselect;

            const curUserData = await getUserInfo(user_ptoken, 'full').catch((e) => {
                console.log(e)
            });
            const locationText = curUserData.full_location;
            const locationValue = curUserData.full_location;
...
function geteventlistdata(queryString, event_type_get = '') {
    getIntaoDb(dbName).then((db) => {
        ...
        const datareventequest = db.transaction(store_name).objectStore(store_name).get(event_list_name);
...
function taoh_events_init(queryString, event_type_get) {
    ...
    if (event_type_get === undefined) event_type_get = "";
    search = $('#query').val();
    geohash = geohashInput.val();
    if (search) {
        searchClear.show();
    ...
    jQuery.get(_taoh_site_ajax_url, data, function (listresponse) {
        console.log('taoh_events_init listresponse:', listresponse, 'get_slug:', get_slug, 'already_rendered:', already_rendered);
        listresponse = parseJSONSafely(listresponse);
        if (window._evl_cfg.intaoDbEnable) {
            console.log('TAOH_INTAODB_ENABLE is true');
            if (!get_slug) {
...
function render_events_template(data, slot, event_type_get) {
    ...
    $.each(result.output.list, function (i, v) {
        ...
        let short_desc = v.short_desc ? v.short_desc : '';

        let user_timezone;
        if (isLoggedIn) {
            user_timezone = window._evl_cfg.userTimezone;
        }
        ...
        let event_url = window._evl_cfg.siteUrlRoot + '/' + window._evl_cfg.appSlug + '/d/' + convertToSlug(taoh_title_desc_decode(v.title)) + '-' + v.eventtoken + '?con=' + v.conttoken;
        let share_event_url = event_url;
        ...
        if (now > eventEndDate) {
            ...
            if (isLoggedIn) {
                ...
                var _chatUrl = window._evl_cfg.siteUrlRoot + '/' + window._evl_cfg.appSlug + '/chat/id/events/' + v.eventtoken;
                var _settingsUrl = window._evl_cfg.siteUrlRoot + '/settings';
                ...
                if (isLive) {
                    ...
                    const liveUrl = is_rsvp_done
                        ? (isValidUser ? _chatUrl : _settingsUrl)
                        : '';
        ...
        var no_image = window._evl_cfg.siteUrlRoot + '/assets/images/event.jpg';
        var img = newavatardisplay(v.user_avatar, v.avatar_image, window._evl_cfg.opsPrefix);
        ...
        slot.append(
            ...
            '<button class="btn shadow-none border-0 social-share-btn" data-toggle="modal" data-target="#socialShareModal" data-url="' + share_event_url + '" data-eventtoken="' + v.eventtoken + '" data-ptoken="' + window._evl_cfg.userPtoken + '" data-title="' + v.title + '" data-desc="' + short_desc + '">' +
...
function get_liked_check(eventtoken, contstoken) {
    if (contstoken === undefined) contstoken = '';
...
function updateCanonical(newCanonicalUrl, event_url) {
    let homeurl = window._evl_cfg.siteUrlRoot + '/' + window._evl_cfg.appSlug + '/';
...
function event_detail_execute(eventtoken, contstoken) {
    ...
    var data = {
        ...
        'ptoken': window._evl_cfg.userPtoken,
    };
...
function indx_events_list(eventlistdata) {
    ...
    var obj_data = {[store_name]: event_taoh_data, [TTLStore]: event_setting_timedata};
...
$(document).on("click", ".event_save", function (event) {
    ...
    $('.events_like').find("[data-cont='" + savetoken + "']").attr('src', window._evl_cfg.siteUrlRoot + '/assets/images/bookmark-fill.svg');
...
$(document).on('click', '.share_box', function (event) {
    ...
    var dat_ajax = window._evl_cfg.siteUrlRoot + '/ajax';
    var image = window._evl_cfg.pageImage;
    var desc = window._evl_cfg.pageDesc;
    var title = window._evl_cfg.pageTitle;
    ...
    $("#share_icon").html(
        '<div class="social-icon-box d-flex text-center" data-ajax="' + dat_ajax + '" data-conttype="' + window._evl_cfg.appSlug + '">' +
...
if (window._evl_cfg.dojoSuggestionEnable) {
    var timelimit = window._evl_cfg.dojoSuggestionTimelimit;
...
$(document).on('click','.social-share-btn', function(event) {
    var dataurl = $(this).attr("data-url");
    var dataeventtoken = $(this).attr("data-eventtoken");
    var dataptoken = $(this).attr("data-ptoken");
    var conttoken = $(this).attr("data-conttoken");
    var pagetitle = $(this).attr("data-title");
    var pagedesc = $(this).attr("data-desc");
    $('#social_event_token').val(dataeventtoken);
    $('#share_link').val(dataurl);
    $('#social_ptoken').val(dataptoken);
...
function saveSearchState() {
    const searchState = {
        query: $('#query').val() || '',
        coordinateLocation: $('#coordinateLocation').val() || '',
        geohash: $('#geohash').val() || '',
        postdate: $('#postdate').val() || '',
        from_date: $('#from_date').val() || '',
        to_date: $('#to_date').val() || '',
    ...
    sessionStorage.setItem('events_search_state', JSON.stringify(searchState));
}
...
function restoreSearchState() {
    ...
    if (searchState.query) {
        $('#query').val(searchState.query);
        search = searchState.query;
        $('#searchClear').show();
    }

    if (searchState.coordinateLocation) {
        $('#coordinateLocation').val(searchState.coordinateLocation);

        let locationSelect = $('#locationSelect')[0]?.tomselect;
        if (locationSelect && searchState.geohash) {
            locationSelect.addOption({
                location: searchState.coordinateLocation,
                coordinates: searchState.coordinateLocation
            });
            locationSelect.setValue(searchState.coordinateLocation);
        }

        $('#geohash').val(searchState.geohash);
        geohash = searchState.geohash;
        $('#locationClear').show();
    }

    if (searchState.postdate) {
        $('#postdate').val(searchState.postdate);
        postDate = searchState.postdate;

        if (searchState.postdate === 'date_range') {
            $('#dateRangeInputs').show();
        }
    }

    if (searchState.from_date) {
        $('#from_date').val(searchState.from_date);
        from_date = searchState.from_date;
    }

    if (searchState.to_date) {
        $('#to_date').val(searchState.to_date);
        to_date = searchState.to_date;
    }
...
function clearSearchState() {
    sessionStorage.removeItem('events_search_state');
}
...
function searchFilter() {
    ...
    geohash = geohashInput.val();
    search = $('#query').val();
    console.log('----event search------', search, queryString);
    ...
    already_rendered = false;
    if (window._evl_cfg.intaoDbEnable) {
        console.log('----intaodb event search------');
        geteventlistdata(queryString, event_type);
    } else {
        console.log('----taoh event search------');
        taoh_events_init('', event_type);
    }
}

/* Override clearBtn for search page version */
function clearBtn(type) {
    loader(true, loaderArea);
    if (type == "search") {
        $('#searchClear').hide();
