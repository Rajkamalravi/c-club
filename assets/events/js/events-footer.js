        ...
        return new Promise((resolve, reject) => {
            ...
            if (serverFetch) {
                $.ajax({
                    ...
                    error: (xhr) => {
                        window.location.href = _taoh_site_url_root + '/404?no=event';
...
async function getSponsorBadges(eventtoken) {
    ...
    if (!data?.values) return {full: [], semi: []};
...
function openSponsorLink(sposnor_id, eventtoken) {
    var link = _taoh_site_url_root + '/events/event_sponsor/' + eventtoken + '/' + sposnor_id + '/update';
...
function getEventLobbyDashboard(eventtoken, response, hallColorArray = {}, search = '') {
    ...
    if (typeof isLoggedIn !== 'undefined' && isLoggedIn) {
        user_timezone = _taoh_user_timezone || '';
    }
                ...
                $.each(speaker_list, function (i, v) {
                    ...
                    if (likedArr && Array.isArray(likedArr.event_speaker) && likedArr.event_speaker.includes(v.ID)) {
                        ...
                        if (v.spk_logo_image != '') {
                            ...
                            spk_img = _taoh_cdn_prefix + "/images/igcache/" + tt + "/630_630/blog.jpg";
                        }
                        ...
                        if (v.enable_tao_networking == 0 && v.spk_external_video_room_link != '') {
                            ...
                            href="${_taoh_site_url_root}/${_taoh_curr_app_slug}/club/${taoh_desc_decode(event_output.conttoken.title)}-${eventtoken}?session_id=${v.ID}&session_name=${encodeURIComponent(v.spk_title)}" target="_blank">Join us</a>`;
                        }
