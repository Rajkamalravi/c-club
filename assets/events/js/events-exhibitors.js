    var my_ptoken = window._exh_cfg.myPtoken;
    var eve = window._exh_cfg.eventToken;
    var sponsorPtokenArray = {};
    // let click_view = 'view';
    var user_country_name = window._exh_cfg.userCountryName;
            ...
            for (let k = 0; k < spon_list.length; k++) {
                ...
                if(v.sponsor_type != undefined && v.sponsor_type != null && !removeSponsor.includes(v.ID) ){
                        ...
                        /* if(v.display_type == 'full')
                                badge= `<img class="lazy sponsor-badge mb-3" src="' + _taoh_site_url_root + '/assets/images/gold.svg" alt="avatar" style="margin-top: -10px;">`;
                            else if(v.display_type == 'semi')
                                badge= `<img class="lazy sponsor-badge mb-3" src="' + _taoh_site_url_root + '/assets/images/silver.svg" alt="avatar" style="margin-top: -10px;">`;
                            else if(v.display_type == 'logos')
                                badge= `<img class="lazy sponsor-badge mb-3" src="' + _taoh_site_url_root + '/assets/images/bronze.svg" alt="avatar" style="margin-top: -10px;">`;
                            */
        ...
        if (!is_content) {
            ...
            if((event_status == 2 || typeof event_status === "undefined" ) && sponsor_type == ''){ // not live and not rsvped as sponsor
                ...
                getEventBaseInfo({ eventtoken: eventToken }, false)
                    ...
                    let social_share_status = window._exh_cfg.socialToken;
                    let trackingtoken = window._exh_cfg.trackingToken;
                    ...
                    if (exh_type == 'exhibitor') {
                        ...
                        jQuery.get(_taoh_site_ajax_url, data, function (response) {
                            ...
                            if (exhibitor_data.sponsor_id != '') {
                                $(".lead-raffle").show();
                                $("#download_raffle").attr('href', _taoh_site_url_root + '/events/export_raffle_entries/?eventtoken=' + eventToken + '&exh=' + exh_id);
                            } else {
                                $(".lead-raffle").hide();
                            }
