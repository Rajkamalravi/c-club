    var my_ptoken = window._ega_cfg.myPtoken;
    var eve = window._ega_cfg.eventToken;
    var app_slug = window._ega_cfg.appSlug;
    var likedArr = window._ega_cfg.likedArr;
    var current_app = window._ega_cfg.currentApp;
    var user_country_name = window._ega_cfg.userCountryName;
    var incomplete_sponsor_form_show = false;
     ...
     window.getEventAgenda = async function getEventAgenda(eventtoken,response,hallColorArray='',search='',tab_name='') {
        ...
        if (isLoggedIn) {
            user_timezone = window._ega_cfg.userTimezone;
        }
       ...
       // $('.for_agenda').hide();
         ...
         getEventBaseInfo({eventtoken}, false)
             .then(async ({requestData, response}) => {
                 ...
                 const isLoggedIn = window._ega_cfg.isLoggedIn;
                 ...
                 if (event_live_state == 'live' || event_live_status == 'live') {
                     event_live_link = (chat_room_status == 2 && isValidUrl(conttoken_data.external_link))
                         ? conttoken_data.external_link : _taoh_site_url_root + '/' + window._ega_cfg.currAppSlug + '/club/' + (conttoken_data.title) + '-' + event_output.eventtoken;
                 ...
                 if (agenda_list != undefined && agenda_list.length > 0) { // added enable_speaker_hall to display speaker list
                     ...
                     $.each(agenda_list, function (i, v) {
                         ...
                         if (v.spk_logo_image != '') {
                             ...
                             spk_img = window._ega_cfg.cdnPrefix + "/images/igcache/" + tt + "/630_630/blog.jpg";
                         ...
                         if (v.enable_tao_networking == 0 && v.spk_external_video_room_link != '') {
                             ...
                             ${disableJoinBtn == 'disabled' ? 'bor-btn' : 'btn-success'} joinus-btn join_networking ${disableJoinBtn}"
                                href="${_taoh_site_url_root}/${window._ega_cfg.currAppSlug}/club/${taoh_desc_decode(event_output.conttoken.title)}-${eventtoken}?session_id=${v.ID}&session_name=${encodeURIComponent(v.spk_title)}" target="_blank">
    ...
    $(document).on("click", ".speaker_save,.exhibitor_save", function (event) {
        ...
        let dataid = `${slug}_id`;
        $('.' + slug + '_like').find(`[data-cont='${savetoken}']`).attr('src', _taoh_site_url_root + "/assets/images/bookmark-fill.svg");
        ...
        var data = {
            ...
            [dataid]: savetoken,
            'ptoken': window._ega_cfg.myPtoken,
        };
