// Events TAO AI - extracted from events_tao_ai_new.php
const _eta = window._eta_cfg || {};

   const isLoggedIn = _eta.isLoggedIn;
   const isValidUser = _eta.isValidUser;

   let loaderArea = $('.listloaderArea');
   let detailloaderArea = $('#detailloaderArea');
   let searchQuery = $('#searchQuery');
   let eventlistArea = $('#eventlistArea');
   let eventlistAreaPopular = $('#eventlistAreaPopular');
   let eventlistAreaFree = $('#eventlistAreaFree');
   let eventlistAreaSoon = $('#eventlistAreaSoon');
   let locationSelectInput = $('#locationSelect');
   let geohashInput = $('#geohash');
   let currentMod = '_eta.appSlug';
   let geohash = "";
   let search = "";
   ...
   let itemsPerPage = 12;
   let currentPage = 1;
   let like_min = '_eta.likeMin';
   let comment_min = '_eta.commentMin';
   let share_min = '_eta.shareMin';
   let app_slug = 'events';
   let arr_cont = [];
   let liked_arr = JSON.parse(`_eta.likedArr`);
   ...
   var event_type = '';
   var user_ptoken = _eta.userPtoken;
   const rsvped_data = _eta.rsvpedData;
   const rsvp_find = new Array();

   let _taoh_static_ajax_token = _eta.staticAjaxToken;
       ...
       getIntaoDb(dbName).then((db) => {
           ...
           datareventequest.onsuccess = () => {
               ...
               if (eventstoredatares !== undefined && eventstoredatares !== null && eventstoredatares !== "" && eventstoredatares !== "undefined" && eventstoredatares !== "null") {
                   ...
                   console.log('------kalpana----eventstoredata----',eventstoredata)
                   render_events_template(eventstoredata, eventlistArea, 'recent');
                   render_events_template(eventstoredata, eventlistAreaPopular, 'popular');
   ...
   function taoh_events_init(queryString = "", event_type_get = "") {
       ...
       jQuery.get(_taoh_site_ajax_url, data, function (listresponse) {
           ...
           if (_eta.intaoDbEnable) {
            ...
            console.log('------kalpana----listresponse----',listresponse)
            if (!already_rendered) {
                   render_events_template(listresponse, eventlistArea, 'recent');
                   render_events_template(listresponse, eventlistAreaPopular, 'popular');
                   render_events_template(listresponse, eventlistAreaFree, 'free');
                   render_events_template(listresponse, eventlistAreaSoon, 'soon');
            }
           } else {
                   render_events_template(listresponse, eventlistArea, 'recent');
                   render_events_template(listresponse, eventlistAreaPopular, 'popular');
   ...
   function render_events_template(data, slot, type) {
       ...
       $('.no_result_div').hide();
           ...
           if(type == 'recent'){
               ...
               var res = data.output.list.popular;
           }else if(type == 'free'){
               var res = data.output.list.free;
           }else if(type == 'soon'){
           ...
           if (data.output === false || data.success === false || final.length === 0) {
                ...
                var htmll = `<div class="row col-lg-12 no_result_div ">
                                <div class="no_result">
                                    <img style="width:20%" src="${_taoh_site_url_root}/assets/images/no_jobs_found.png" alt="No Results Illustration">
   ...
   //////////////////////////////////////Near/////////////////////////////////
       $.each(final, function (i, v) {
            //alert(v.eventtoken)
            if(v.eventtoken == undefined){
                return;
           ...
           let user_timezone;
           if (isLoggedIn) {
               user_timezone = _eta.userTimezone;
           }
           ...
           let event_url = `${source}/' + _eta.appSlug + '/d/'${convertToSlug(taoh_title_desc_decode(v.title))}-${v.eventtoken}?con=${v.conttoken}`;
               ...
               if (jQuery.inArray(rsvped_token, rsvp_find) !== -1 || isEventTokenPresent(v.eventtoken)) {
                   ...
                   if (event_live_state == 'live') {
                       if (isValidUser) {
                           ...
                           btn_icon = '<i class="fa fa-rss mr-2" aria-hidden="true"></i>';
                           event_url = `${v.source}/' + _eta.appSlug + '/chat/id/events/'${v.eventtoken}`; // lobby url
                       } else {
                           btn_text = 'Complete Settings to Join';
                           btn_class = 'btn-success';
                           btn_icon = '<i class="fa fa-cogs mr-2" aria-hidden="true"></i>';
                           event_url = '${_taoh_site_url_root}/settings'; // settings url
                       }
                   } else {
           ...
           var img = newavatardisplay(v.user_avatar, v.avatar_image, _eta.opsPrefix);
   ...
   $(document).on('click', '.share_box', function (event) {
       ...
       var image = _eta.pageImage;
       var desc = _eta.pageDesc;
       var title = _eta.pageTitle;
   ...
   <div class="social-icon-box d-flex text-center" data-ajax="${(dat_ajax)}" data-conttype="_eta.appSlug">
