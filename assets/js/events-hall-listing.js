var _ehl = window._ehl_cfg || {};
const isLoggedIn = _ehl.isLoggedIn;
const isValidUser = _ehl.isValidUser;
let is_user_rsvp_done = _ehl.isUserRsvpDone;
let eventtoken = _ehl.eventtoken;
let eventhall = _ehl.eventhall;
let TAOH_CURR_APP_URL = _ehl.currAppUrl;
let event_output;
let user_timezone;
let local_timezone;
const user_profile_type = _ehl.userProfileType;
...
if (isLoggedIn) {
    user_timezone = _ehl.userTimezone;
}
...
$(document).ready(function () {
    getEventBaseInfo({eventtoken}, false)
        .then(({requestData, response}) => {
            ...
            const mediaHtml = '<div class="item">' +
            ...
            if (event_live_state == 'live') {
                event_live_link = (chat_room_status == 2 && isValidUrl(event_output.conttoken.external_link))
                    ? event_output.conttoken.external_link : _taoh_site_url_root + '/' + _ehl.appSlug + '/club/' + (event_output.conttoken.title) + '-' + event_output.eventtoken;
                if (chat_room_status) {
...
function getHallDetails(eventtoken) {
    var content = '';
    var eventMetaInfoBaseInfoKey = 'event_MetaInfo_' + eventtoken;
    var rsvp_sponsor_title = _ehl.rsvpSponsorTitle;
    var is_user_rsvp_done = _ehl.isUserRsvpDone;
            ...
            if (filteredSpeakers != undefined && filteredSpeakers.length > 0) { // added enable_speaker_hall to display speaker list
                ...
                $.each(filteredSpeakers, function (i, v) {
                    ...
                    if (v.spk_logo_image != '') {
                        ...
                        spk_img = _ehl.cdnPrefix + "/images/ig/" + encodeURIComponent(v.spk_title) + "/uncategorized/1.png";
                    }
                    ...
                    content += '<a style="width:30px;" id="dropdown-agenda_' + i + '" data-target="#agenda-content_' + i + '" class="svg-opt-con btn dropdown-agenda">' +
                    ...
                    ' href="' + _taoh_site_url_root + '/' + _ehl.appSlug + '/club/' + taoh_desc_decode(event_output.conttoken.title) + '-' + eventtoken + '?session_id=' + v.ID + '&session_name=' + encodeURIComponent(v.spk_title) + '" target="_blank">Join us</a>';
                    }
...
function getEventsHall(eventtoken) {
    const eventdetailBaseInfoKey = 'event_detail_' + _ehl.eventtoken;
...
async function getEventsHallList(response) {
    ...
    var user_profile_type = $("#user_profile_type").val();
    var rsvp_sponsor_title = $("#rsvp_sponsor_title").val();
    let hall_option_speaker = '<a class="dropdown-item" onclick="LoadMetaWithHall(\'event_speaker\',\'\',\'All\');">All</a>  ';
