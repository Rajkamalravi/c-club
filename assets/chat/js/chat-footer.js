...
async function clearCacheProcess(cacheData) {
    ...
    for (let [key, item] of Object.entries(cacheData)) {
        ...
        if (item.action === 'channel_add') {
            if (Object.keys(item.value).length > 0) {
                const value = item.value;

                let room_hash = value.room;
                let channelId = value.channel_id;
                let channelType = value.channel_type;

                let ntw_channels_key = 'ntw_channels_' + room_hash;
                const intaoData = await IntaoDB.getItem(objStores.ntw_store.name, ntw_channels_key);
                let channelList = intaoData ? intaoData.values : {};

                if (channelList.channels && Object.keys(channelList.channels).length > 0 && !channelList.channels[channelId]) {
                    let data = {
                        ops: 'channel',
                        action: 'get',
                        code: _taoh_ops_code,
                    ...
                    $.ajax({
                        ...
                        success: function (response, textStatus, jqXHR) {
                            if (response.success && response.data) {
                                channelList[channelId] = response.channel;
        ...
        if (item.action === 'channel_message_added') {
            if (Object.keys(item.value).length > 0) {
                const value = item.value;
                ...
                let channelId = value.channel_id ?? '';
                let channelType = value.channel_type ?? '';
                let frm_message_id = parseInt(value.parent_id, 10);

                const isReplyRequest = Boolean(parseInt(value.parent_id, 10));

                if (channelType == 3) { // 3 - Only channel
                    if (_ntw_ft_ptoken && room_hash) {
                        const chat_networking_misc_key = `ft_frm_networking_misc_${room_hash}${isReplyRequest ? '_' + frm_message_id : ''}`;
                        IntaoDB.getItem(objStores.ntw_store.name, chat_networking_misc_key).then((intao_data) => {
                            if (isReplyRequest) {
                                lastFRMReplyMsgCheckedTimestamp = (intao_data && intao_data.last_update_time) ? intao_data.last_update_time : 0;
                                getAndSetLastCheckedTimestamp('lastFRMReplyMsgCheckedTimestamp', lastFRMReplyMsgCheckedTimestamp, 2);
                            } else {
                                lastFRMMsgCheckedTimestamp = (intao_data && intao_data.last_update_time) ? intao_data.last_update_time : 0;
                            ...
                            if (isReplyRequest) {
                                ...
                                taohFRMMessagesFromServer(getForumMessagesFormData(room_hash, _ntw_ft_ptoken, frm_message_id, 1, lastCheckedTimestamp));
                            } else {
                                const lastCheckedTimestamp = getAndSetLastCheckedTimestamp('lastFRMMsgCheckedTimestamp', lastFRMMsgCheckedTimestamp, 1);
                                taohFRMMessagesFromServer(getForumMessagesFormData(room_hash, _ntw_ft_ptoken, 0, 1, lastCheckedTimestamp));
                            }
                        });
                    }
                }
        ...
        if (item.action === 'like_message') {     
            ...
            if (Object.keys(item.value).length > 0) {
                ...
                let channelId = value.channel_id ?? '';
                let channelType = value.channel_type ?? '';
                let frm_message_id = parseInt(value.message_id, 10);
        ...
        if (item.action === 'direct_message_added') {
            /*if (Object.keys(item.value).length > 0) {
                const value = item.value;
                ...
                if (channelType == 0) { // 0 - Only One to One Conversation
                    if (_ntw_ft_ptoken && room_hash) {
                        const chat_networking_misc_key = `ft_ntw_networking_misc_${room_hash}`;
                        IntaoDB.getItem(objStores.ntw_store.name, chat_networking_misc_key).then((intao_data) => {
                            lastNTWMsgCheckedTimestamp = (intao_data && intao_data.last_update_time) ? intao_data.last_update_time : 0;
                            ...
                            taohNTWMessagesFromServer(getNetworkingMessagesFormData('ntw', room_hash, _ntw_ft_ptoken, 0, lastCheckedTimestamp));
                        });
                    }
                }
            }*/
        ...
        if (item.action === 'forum_delete_message') {
            if (Object.keys(item.value).length > 0) {
                ...
                let parent_id = parseInt(value.parent_id, 10) || 0;
                let isReplyRequest = Boolean(parent_id);

                let chat_messages_key = `frm_${room_hash}_${channel_id}${isReplyRequest ? '_' + parent_id : ''}`;
                ...
                if (chatMessages?.chat && Object.keys(chatMessages.chat).length > 0 && chatMessages.chat[message_key]) {
...
function getCacheKeyDataToProcess(requestData) {
    new Promise((resolve, reject) => {
            $.ajax({
                    ...
                    error: function (xhr, status, error) {
                        console.error('Error fetching cache key data:', error);
                        reject(error);
                    }
                }
            );
...
async function speedNetworkingProcess(cacheData) {
    ...
    for (let [key, item] of Object.entries(cacheData)) {
        ...
        //         const value = item.value;
        //         let room_hash = value.room;
        //         let ptoken = value.ptoken;
        //         speedNetworkingAddUser(room_hash, ptoken);
        //     }
        //     alreadyProcessed = true;
        //     allowCacheFilesToRemove = false;
        // }
        ...
        // if (item.action === 'speed_networking_connect_update') {
        //     if (Object.keys(item.value).length > 0) {
        //         const value = item.value;
        //         let room_hash = value.room;
        //         let chat_from = value.chat_from;
        ...
        // if (item.action === 'speed_networking_block_user') {
        //     if (Object.keys(item.value).length > 0) {
...
function initializeRequest(chat_window, room_hash, my_pToken) {
    if (chat_window === 'channel') {
        if (frm_reply_view) {
            const lastCheckedTimestamp = getAndSetLastCheckedTimestamp('lastFRMReplyMsgCheckedTimestamp', lastFRMReplyMsgCheckedTimestamp, 1);
            taohFRMMessagesFromServer(getForumMessagesFormData(room_hash, my_pToken, frm_message_id, 0, lastCheckedTimestamp));
        } else {
            const lastCheckedTimestamp = getAndSetLastCheckedTimestamp('lastFRMMsgCheckedTimestamp', lastFRMMsgCheckedTimestamp, 1);
            taohFRMMessagesFromServer(getForumMessagesFormData(room_hash, my_pToken, 0, 0, lastCheckedTimestamp));
        }

        if(typeof ntwUserEntriesIntervalId !== 'undefined' && ntwUserEntriesIntervalId) clearInterval(ntwUserEntriesIntervalId);
    } else if (chat_window === 'direct_message') {
        const lastCheckedTimestamp = getAndSetLastCheckedTimestamp('lastNTWMsgCheckedTimestamp', lastNTWMsgCheckedTimestamp, 1);
        taohNTWMessagesFromServer(getNetworkingMessagesFormData('ntw', room_hash, my_pToken, 0, lastCheckedTimestamp));

        // updateNTWUserEntriesInterval();

    }  else if (chat_window === 'private_message') {
        chat_activeTabId = 'direct_message'; // for footer fn
        const lastCheckedTimestamp = getAndSetLastCheckedTimestamp('lastPCMsgCheckedTimestamp', lastPCMsgCheckedTimestamp, 1);
        taohNTWMessagesFromServer(getNetworkingMessagesFormData('pc', room_hash, my_pToken, 0, lastCheckedTimestamp));

    } else if (chat_window === 'rooms') {
        if(typeof ntwUserEntriesIntervalId !== 'undefined' && ntwUserEntriesIntervalId) clearInterval(ntwUserEntriesIntervalId);
    }
}
