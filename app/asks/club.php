...
@$conttoken = array_pop( explode( '-', $contslug ) );
...
if ( isset( $_GET[ 'key' ] ) && $_GET[ 'key' ] ){
    ...
    $ask_arr = $ask_arr[ 'output' ];
    @$country = array_pop( explode( ', ',$ask_arr[ 'meta' ][ 'full_location' ] ) );
    $keyslug = hash('crc32', $contslug.$country );
}

$room_status = get_room_info( $keyslug, $sess_user_info->ptoken, ['app' => 'ask']);
$room_status_arr = json_decode( $room_status, true );
//print_r($ask_arr);exit();
if ( isset( $room_status_arr[ 'output' ] ) && $room_status_arr[ 'output' ] ){
    ...
    if ( isset( $owner_arr[ 'output' ] ) && $owner_arr[ 'output' ] ){    
        ...
        taoh_networking_postcell( $owner_arr_cell[ 'cell_info' ] ,$owner_arr[ 'ptoken' ]);
    }
    //print_r($owner_arr);exit();
    $return = array(
        'keyslug' => $keyslug,
        ...
        'club' => array(
            'title' => $ask_arr[ 'title' ],
            'description' => urldecode($ask_arr[ 'description' ]),
            'short' => substr(strip_tags( urldecode($ask_arr[ 'description' ]) ), 0, 300),
            'image' => taoh_either_or( $ask_arr[ 'meta' ][ 'image' ], TAOH_CURR_APP_IMAGE ),
            'square_image' => taoh_either_or( $ask_arr[ 'meta' ][ 'image_sq' ], TAOH_CURR_APP_IMAGE_SQUARE ),
            'links' => array(
                'club' => '/club/room/'.taoh_slugify($ask_arr[ 'title' ]).'-'.$keyslug,
            ...
            'skill' => taoh_either_or( $ask_arr[ 'meta' ][ 'skill' ], '' ),
            'company' => taoh_either_or( $ask_arr[ 'meta' ][ 'company' ], '' ),
            'roles' => taoh_either_or( $ask_arr[ 'meta' ][ 'title' ], '' ),
            'sponsors' => array(
                array(
            ...
            'breadcrumbs' => array(
                ...
                // array(
                //     'title' => 'Networking',
                //     'link' => '/'.TAOH_SITE_CURRENT_APP_SLUG.'/'.taoh_slugify($ask_arr[ 'title' ]).'-'.$keyslug,
                // ),
            ),
            'live' => '',
            'geo_enable' => false,
            ...
            'full_location' => taoh_either_or( $ask_arr[ 'meta' ][ 'full_location' ], '' ),
            'coordinates' => taoh_either_or( $ask_arr[ 'meta' ][ 'coordinates' ], '' ),
            'geohash' => taoh_either_or( $ask_arr[ 'meta' ][ 'geohash' ], '' ),
            'longitude' => '',
            'latitude' => '',
            'faq' => array(
                array(
...
//$url = TAOH_SITE_URL_ROOT.$room_status_arr[ 'output' ][ 'club' ][ 'links' ][ 'club' ].'/CHATwith/'.$return[ 'club_info' ][ 'club' ][ 'owner' ][ 'ptoken' ];
