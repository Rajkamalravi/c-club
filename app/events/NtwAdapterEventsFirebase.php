...
final class NtwAdapterEventsFirebase
{
    ...
    public function constructAndCreateRoomInfo($sess_user_info, $input_data = [])
    {
        ...
        /*$create_room_response = $this->createRoomInfoOnFirebase($room_info_arr, $eventtoken, $ptoken);
    ...
    public function updateRoomInfo($sess_user_info, $input_data = [])
    {
        ...
        $update_room_response['room_data'] = $room_info_arr;
        return $update_room_response;
    }

    public function createRoomInfoOnFirebase($room_data, $keyword, $ptoken = '', $data = array())
    {
            ...
            $log_dir = dirname(__DIR__) . '/events';
            file_put_contents($log_dir . '/room_data.log', "=== " . date('Y-m-d H:i:s') . " === createRoomInfoOnFirebase INPUT ===\nkeyslug: " . ($room_data['room']['keyslug'] ?? 'EMPTY') . "\neventtoken: " . $keyword . "\nptoken: " . $ptoken . "\nPOST value length: " . strlen($taoh_vals['value']) . "\n\n", FILE_APPEND);
            $room_data_json = taoh_post(TAOH_FIREBASE_SCRIPT_URL, $taoh_vals);
            file_put_contents($log_dir . '/room_data.log', "=== " . date('Y-m-d H:i:s') . " === createRoomInfoOnFirebase RAW RESPONSE ===\n" . substr($room_data_json, 0, 2000) . "\n\n", FILE_APPEND);
