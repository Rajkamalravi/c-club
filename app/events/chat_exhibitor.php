...
$sess_user_info = taoh_session_get(TAOH_ROOT_PATH_HASH)['USER_INFO'];
    ...
    if (isset($sess_user_info->local_timezone) && !empty($sess_user_info->local_timezone)) {
    ...
    if ( ( ! isset( $response[ 'success' ] ) || ( isset( $response[ 'success' ] ) && ! $response[ 'success' ] ) ) ){
     ...
     $exhibitor_data = $response['output'];

    $session_title = $exhibitor_data['exh_session_title'];
    ...
    $country = '';
    if(!empty($sess_user_info?->full_location)){
        $sess_full_loc = explode(', ', $sess_user_info->full_location);
        $country = array_pop($sess_full_loc);
    }

    $abbreviation = '';
    if(!empty($sess_user_info?->local_timezone)){
        try {
    ...
    //echo "============".$geo_enable;die();
    //echo '<pre>';print_r($exhibitor_data);exit();
    $title = $title_desc = '';

    $title = $session_title ;
    $title_desc = $exhibitor_data['exh_description'];
    ...
    $event_path = '/'.TAOH_SITE_CURRENT_APP_SLUG.'/d/'.$events_arr[ 'conttoken' ][ 'title' ].'-'.$eventtoken;
     ...
     $room_info_arr = array(
        ...
        'club' => array(
            ...
            'short' => urldecode( $exhibitor_data['exh_description']),
            'streaming_link' => taoh_either_or( $exhibitor_data['exh_streaming_link'], '' ),
            'image' => taoh_either_or( $exhibitor_data['exh_logo'], TAOH_CURR_APP_IMAGE ),
            'square_image' => taoh_either_or( $exhibitor_data['exh_logo'], TAOH_CURR_APP_IMAGE_SQUARE ),
            'links' => array(
            ...
            'exhibitor_data' => json_encode($exhibitor_data),
